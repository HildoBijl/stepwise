...
describe('validateAndProcessPrerequisites', () => {
	it('creates continuation IDs for chains and branches in tree order', () => {
		const tree = flattenSkillTreeDefinition({ a: { name: 'A' }, b: { name: 'B', prerequisites: ['a'] }, c: { name: 'C', prerequisites: ['a'] }, d: { name: 'D', prerequisites: ['b', 'c'] } })
	...
	it('rejects unknown prerequisites before modifying any continuation IDs', () => {
		const tree = flattenSkillTreeDefinition({ a: { name: 'A' }, b: { name: 'B', prerequisites: ['a'] }, c: { name: 'C', prerequisites: ['missing'] } })
	...
	it('rejects direct and longer prerequisite cycles', () => {
		expect(() => validateAndProcessPrerequisites(flattenSkillTreeDefinition({ a: { name: 'A', prerequisites: ['a'] } }))).toThrow('"a" -> "a"')
		expect(() => validateAndProcessPrerequisites(flattenSkillTreeDefinition({ a: { name: 'A', prerequisites: ['b'] }, b: { name: 'B', prerequisites: ['c'] }, c: { name: 'C', prerequisites: ['a'] } }))).toThrow('"a" -> "b" -> "c" -> "a"')
	})
})
