import { describe, expect, it } from 'vitest'

import { and } from '@step-wise/skill-setup'

import { flattenSkillTreeDefinition } from './flattening.ts'
import type { SkillTreeDefinition } from './types.ts'

describe('flattenSkillTreeDefinition', () => {
	it('flattens nested groups and derives paths and group skill IDs', () => {
		const tree = flattenSkillTreeDefinition({ subject: { group: { a: { name: 'A' }, b: { name: 'B' } }, c: { name: 'C' } } })
		expect(Object.keys(tree)).toEqual(['a', 'b', 'c'])
		expect(tree.a).toMatchObject({ id: 'a', name: 'A', groupPath: ['subject', 'group'], groupModuleIds: ['a', 'b'] })
		expect(tree.b.groupModuleIds).toBe(tree.a.groupModuleIds)
		expect(tree.c).toMatchObject({ groupPath: ['subject'], groupModuleIds: ['c'] })
	})

	it('keeps structurally different groups separate when their paths serialize equally', () => {
		const tree = flattenSkillTreeDefinition({ 'a/b': { first: { name: 'First' } }, a: { b: { second: { name: 'Second' } } } })
		expect(tree.first.groupModuleIds).toEqual(['first'])
		expect(tree.second.groupModuleIds).toEqual(['second'])
		expect(tree.first.groupModuleIds).not.toBe(tree.second.groupModuleIds)
	})

	it('combines and deduplicates explicit and setup-derived prerequisites', () => {
		const setup = and('a', 'b')
		const tree = flattenSkillTreeDefinition({ a: { name: 'A' }, b: { name: 'B' }, c: { name: 'C', prerequisites: ['a'], setup, thresholds: { mastery: 0.7 } } })
		expect(tree.c.prerequisiteIds).toEqual(['a', 'b'])
		expect(tree.c.setup).toBe(setup)
		expect(tree.c.thresholds).toMatchObject({ mastery: 0.7, recap: 0.63, priorKnowledgeMastery: 0.7 })
		expect(tree.c.thresholds.priorKnowledgeRecap).toBeCloseTo(0.56)
		expect(tree.c).toMatchObject({ continuationIds: [], linkedSkillIds: [] })
	})

	it('normalizes preliminary links', () => {
		const tree = flattenSkillTreeDefinition({ a: { name: 'A', links: { skillId: 'b', correlation: 0.5 } }, b: { name: 'B' } })
		expect(tree.a.links).toEqual([{ skillIds: ['b'], correlation: 0.5 }])
	})

	it('returns a prototype-free tree and supports special skill IDs', () => {
		const rawTree = Object.create(null) as SkillTreeDefinition
		Object.defineProperty(rawTree, 'toString', { value: { name: 'To string' }, enumerable: true })
		Object.defineProperty(rawTree, 'constructor', { value: { name: 'Constructor' }, enumerable: true })
		Object.defineProperty(rawTree, '__proto__', { value: { name: 'Prototype' }, enumerable: true })
		const tree = flattenSkillTreeDefinition(rawTree)
		expect(Object.getPrototypeOf(tree)).toBeNull()
		expect(Object.keys(tree)).toEqual(['toString', 'constructor', '__proto__'])
	})

	it('allows a group to contain a skill whose ID is name', () => {
		const tree = flattenSkillTreeDefinition({ group: { name: { name: 'Name' }, other: { name: 'Other' } } })
		expect(tree.name.groupPath).toEqual(['group'])
		expect(tree.name.groupModuleIds).toEqual(['name', 'other'])
	})

	it('rejects exact and case-insensitive duplicate IDs with their paths', () => {
		expect(() => flattenSkillTreeDefinition({ first: { a: { name: 'First' } }, second: { a: { name: 'Second' } } })).toThrow(/first\/a.*second\/a|second\/a.*first\/a/)
		expect(() => flattenSkillTreeDefinition({ first: { Skill: { name: 'First' } }, second: { skill: { name: 'Second' } } })).toThrow(/regardless of casing/)
	})

	it('rejects empty skill IDs and names', () => {
		expect(() => flattenSkillTreeDefinition({ '': { name: 'Empty ID' } })).toThrow(RangeError)
		expect(() => flattenSkillTreeDefinition({ a: { name: '' } })).toThrow(RangeError)
		expect(() => flattenSkillTreeDefinition({ a: { name: '   ' } })).toThrow(RangeError)
	})

	it('rejects skill IDs and references with surrounding whitespace', () => {
		expect(() => flattenSkillTreeDefinition({ ' a': { name: 'A' } })).toThrow(/start or end with whitespace/)
		expect(() => flattenSkillTreeDefinition({ 'a ': { name: 'A' } })).toThrow(/start or end with whitespace/)
		expect(() => flattenSkillTreeDefinition({ a: { name: 'A', prerequisites: [' b'] } })).toThrow(/start or end with whitespace/)
		expect(() => flattenSkillTreeDefinition({ a: { name: 'A', links: 'b ' } })).toThrow(/start or end with whitespace/)
		expect(() => flattenSkillTreeDefinition({ a: { name: 'A', setup: and(' b', 'c') } })).toThrow(/start or end with whitespace/)
	})

	it('validates raw skill properties', () => {
		expect(() => flattenSkillTreeDefinition({ a: { name: 'A', prerequisites: 'b' } } as unknown as SkillTreeDefinition)).toThrow(/prerequisites.*array/)
		expect(() => flattenSkillTreeDefinition({ a: { name: 'A', setup: {} } } as unknown as SkillTreeDefinition)).toThrow(/setup.*SkillSetup/)
		expect(() => flattenSkillTreeDefinition({ a: { name: 'A', thresholds: 0.5 } } as unknown as SkillTreeDefinition)).toThrow(/threshold.*plain object/)
	})

	it.each([NaN, -0.1, 1.1, Infinity, '0.5'])('rejects an invalid mastery threshold: %s', mastery => {
		expect(() => flattenSkillTreeDefinition({ a: { name: 'A', thresholds: { mastery } } } as unknown as SkillTreeDefinition)).toThrow()
	})

	it.each([0, 0.5, 1])('accepts a mastery threshold on the inclusive unit interval: %s', mastery => {
		expect(flattenSkillTreeDefinition({ a: { name: 'A', thresholds: { mastery } } }).a.thresholds.mastery).toBe(mastery)
	})

	it('rejects malformed group entries with their path', () => {
		expect(() => flattenSkillTreeDefinition({ group: { broken: 3 } } as unknown as SkillTreeDefinition)).toThrow(/group\/broken/)
	})
})
