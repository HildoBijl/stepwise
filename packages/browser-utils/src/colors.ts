...
function lighten(color: ColorInput, part: number): Color {
...
function darken(color: ColorInput, part: number): Color {
	return invert(lighten(invert(color), part))
}

function invert(color: ColorInput): Color {
	return ensureColor(color).map((value, index) => index === 3 ? value : 1 - value) as Color
}
