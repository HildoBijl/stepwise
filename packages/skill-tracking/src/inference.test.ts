...
import { coefficientsToStoredSkillLevel, effectivelyInfinitePracticeCount, now, moduleTree, twoMonthsAgo } from './testUtils.ts'
...
describe('elementary skill inference', () => {
	...
	it('applies practice decay', () => {
		const levels = new SkillLevelSet(moduleTree, { a: coefficientsToStoredSkillLevel([0, 1]) })
	...
	it('applies time decay', () => {
		const levels = new SkillLevelSet(moduleTree, { a: coefficientsToStoredSkillLevel([0, 1], twoMonthsAgo, effectivelyInfinitePracticeCount) })
		const expectedSuccessRate = levels.getExpectedSuccessRate('a')
		expect(expectedSuccessRate).toBeGreaterThan(1 / 2)
		expect(expectedSuccessRate).toBeLessThan(2 / 3)
	})
	...
	it('treats future dates as zero elapsed time', () => {
		...
		const levels = new SkillLevelSet(moduleTree, { a: coefficientsToStoredSkillLevel([0, 1], future, effectivelyInfinitePracticeCount) })
		expect(levels.getInferredCoefficients('a')).toEqual([0, 1])
	})
})

describe('setup inference', () => {
	const levels = () => new SkillLevelSet(moduleTree, {
	...
	it('preserves uncertainty instead of substituting only the mean', () => {
		const uniformLevels = new SkillLevelSet(moduleTree, { a: coefficientsToStoredSkillLevel([1], now, effectivelyInfinitePracticeCount) })
	...
	it('throws when required skill data is missing', () => {
		const incomplete = new SkillLevelSet(moduleTree, { a: coefficientsToStoredSkillLevel([1]) })
