...
describe('construction and access', () => {
	...
	it('rejects unknown stored skills and access to unloaded levels', () => {
		expect(() => new SkillLevelSet(moduleTree, { unknown: coefficientsToStoredSkillLevel([1]) })).toThrow()
		expect(() => new SkillLevelSet(moduleTree).getSkillLevel('a')).toThrow()
	})
})
...
describe('updates', () => {
	...
	it('accepts newer updates and ignores stale or duplicate updates', () => {
		const levels = new SkillLevelSet(moduleTree, { a: coefficientsToStoredSkillLevel([1], now, 2) })
