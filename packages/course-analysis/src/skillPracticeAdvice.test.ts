...
import { courseDefinition } from './testUtils.ts'
...
describe('getSkillPracticeAdvice', () => {
	it('returns undefined without a course analysis', () => {
		expect(getSkillPracticeAdvice(courseDefinition, undefined, 'basic')).toBeUndefined()
	})

	it('stays in free practice when only recommended practice remains', () => {
		const analysis = createAnalysis({ basic: 1 }, 'basic')
		expect(getSkillPracticeAdvice(courseDefinition, analysis)).toEqual({ type: 'stay', recommendation: freePracticeRecommendation })
	})

	it('leaves free practice for a skill that requires practice', () => {
		const analysis = createAnalysis({ basic: 2 }, 'basic')
		expect(getSkillPracticeAdvice(courseDefinition, analysis)).toEqual({ type: 'goBack', recommendation: 'basic' })
	})

	it('identifies a supplied skill outside the course', () => {
		const analysis = createAnalysis({ basic: 1 }, 'basic')
		expect(getSkillPracticeAdvice(courseDefinition, analysis, 'outside')).toEqual({ type: 'notInCourse', recommendation: 'basic' })
	})

	it('stays on a skill for which practice is recommended', () => {
		const analysis = createAnalysis({ basic: 1 }, 'basic')
		expect(getSkillPracticeAdvice(courseDefinition, analysis, 'basic')).toEqual({ type: 'stay', recommendation: 'basic' })
	})
	...
	it('goes back recursively to the earliest deficient prerequisite', () => {
		...
		expect(getSkillPracticeAdvice(courseDefinition, analysis, 'intermediate')).toEqual({ type: 'goBack', recommendation: 'foundation' })
	})
	...
	it('moves onward through mastered continuations to a skill needing practice', () => {
		...
		expect(getSkillPracticeAdvice(courseDefinition, analysis, 'basic')).toEqual({ type: 'moveOnward', recommendation: 'advanced' })
	})
	...
	it('skips continuation skills without exercises', () => {
		...
		expect(getSkillPracticeAdvice(courseDefinition, analysis, 'basic', hasExercises)).toEqual({ type: 'moveOnward', recommendation: 'advanced' })
	})
	...
	it('moves onward to free practice when no continuation needs practice', () => {
		...
		expect(getSkillPracticeAdvice(courseDefinition, analysis, 'advanced')).toEqual({ type: 'moveOnward', recommendation: freePracticeRecommendation })
	})
})
