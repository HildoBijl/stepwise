...
export type EnsureIntegerOptions = EnsureNumberOptions & {
	safe?: boolean
}
...
export function ensureInteger(value: unknown, options: EnsureIntegerOptions = {}): number {
	...
	if (options.safe && !Number.isSafeInteger(integer)) throw new RangeError(`Input error: expected a safe integer but received "${integer}".`)
...
export function ensureNumericInteger(value: unknown, options: EnsureIntegerOptions = {}): number {
