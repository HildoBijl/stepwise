...
export default buildStepExercise({
	...
	getSolution(parameters) {
		...
		const fractions = ['(a*x*(x+b))/(fx)', '(c*x^2+d*x+e)/(f*x)'].map(str => asExpression(str, { interpretEAsConstant: false }).substitute(variables).removeTrivial([], ['mergeFractionMinuses']))
