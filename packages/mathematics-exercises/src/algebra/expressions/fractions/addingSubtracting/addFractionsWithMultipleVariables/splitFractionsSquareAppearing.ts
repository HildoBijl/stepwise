...
export default buildStepExercise({
	...
	getSolution(parameters) {
		...
		const expression = asExpression(`(ay^2${sign}bxz)/(xy)`).substitute(variables).removeTrivial(['combineNumbersInProducts', 'sortProducts'])
		const leftExpression = asExpression('(ay^2)/(xy)').substitute(variables).removeTrivial(['combineNumbersInProducts', 'sortProducts'])
		const rightExpression = asExpression('(bxz)/(xy)').substitute(variables).removeTrivial(['combineNumbersInProducts', 'sortProducts'])
