...
export default buildStepExercise({
	metadata: {
		...
		comparisons: {
			withoutNegativeExponents: (input: Expression, correct: Expression) => !hasNegativeExponent(input) && areEquivalent(input, correct),
	...
	checkInput(data, step) {
		switch (step) {
			case 1: return compareInputs('withoutNegativeExponents', data)
			case 2: return compareInputs('singleFraction', data)
