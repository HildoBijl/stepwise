...
import type { InputExerciseHistoryInstance, InputExerciseInput, InputExerciseInputValue } from '../InputExercise'
...
export function getLastRawInputAtStep(instance: InputExerciseHistoryInstance<StepExerciseState>, step: number, userId?: string, requireResolved = false): InputExerciseInputValue | undefined {
	...
	if (instance.mode === 'solo') for (let index = instance.history.length - 1; index >= 0; index--) {
		...
		if (getStep(getStateBeforeEvent(instance, index)) === step) return userAction.input
	}
...
export function getLastInputAtStep(instance: InputExerciseHistoryInstance<StepExerciseState>, step: number, userId?: string, requireResolved = false): InputExerciseInput | undefined {
	const rawInput = getLastRawInputAtStep(instance, step, userId, requireResolved)
