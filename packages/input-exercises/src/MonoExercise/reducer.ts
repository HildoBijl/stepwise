...
import { type GroupInputExerciseReport, type InputDependency, type InputExerciseAction, type InputExerciseParameters, type InputExerciseReport, type InputExerciseSolution, type InputExerciseValueOperations, type SoloInputExerciseReport, resolveSolution, resolveStaticSolution, resolveUpdatedInputDependency } from '../InputExercise/index.ts'
...
async function reduceActions<TParameters extends InputExerciseParameters, TSolution extends InputExerciseSolution, TInputDependency>(spec: MonoExerciseSpec<TParameters, TSolution, TInputDependency>, reducerInput: InputExerciseReducerInput<InputExerciseAction, MonoExerciseState, TParameters>, valueOperations: InputExerciseValueOperations): Promise<{ state: MonoExerciseState, reports: (InputExerciseReport | undefined)[] }> {
	...
	const results = await Promise.all(actions.map(async userAction => {
		...
		const result = normalizeCheckInputResult(await checkInput({ metadata, parameters, rawInput: userAction.action.input, input: exerciseInput, solution, areValuesEqual: valueOperations.areValuesEqual }))
