...
describe('bracket matching', () => {
	test('finds top-level matching brackets while ignoring nested pairs', () => {
		expect(getTopLevelBracketMatches(['(x+(y))'])).toEqual([{ opening: { part: 0, cursor: 0 }, closing: { part: 0, cursor: 6 } }])
	})
	...
	test('finds separate top-level bracket pairs across expression parts', () => {
		...
		expect(getTopLevelBracketMatches(value)).toEqual([
	...
	test('treats a logarithm construct as an opening round bracket', () => {
		...
		expect(getTopLevelBracketMatches(value)).toEqual([{ opening: { part: 1, cursor: 0 }, closing: { part: 2, cursor: 5 } }])
		expect(findClosingBracket(value, { part: 0, cursor: 0 })).toEqual({ part: 2, cursor: 6 })
	})

	test('reports unmatched opening and closing brackets', () => {
		expect(() => getTopLevelBracketMatches(['(x'])).toThrow('missing closing bracket')
		expect(() => getTopLevelBracketMatches(['x)'])).toThrow('missing opening bracket')
	})
})
