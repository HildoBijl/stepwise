...
import { type AnyValueEqualityAdapter, type ValueEqualityOptions, type ValueEqualityAdapter, areValuesEqual } from '@step-wise/value-equality'
...
export function compareInputValue(inputValue: unknown, expectedValue: unknown, options: InputValueComparisonOptions): boolean {
	...
	const equality = getEqualityAdapter(type, data)
...
function getEqualityAdapter(type: string, data: CheckInputData): AnyValueEqualityAdapter | undefined {
	if (Object.hasOwn(data.equalityAdapters, type)) return data.equalityAdapters[type]
	return equalityAdapters[type as keyof typeof equalityAdapters]
}
