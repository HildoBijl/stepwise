...
import { combineValueTypes, extractValueTypeAdapters } from './registries.ts'
...
describe('value-type registries', () => {
	it('extracts each adapter capability independently', () => {
		...
		expect(combinedAdapters.inputValueAdapters).toEqual({ [ExampleType]: inputValueAdapter, InputOnly: inputValueAdapter })
		expect(combinedAdapters.serializationAdapters).toEqual({ [ExampleType]: serializationAdapter, SerializationOnly: serializationAdapter })
		expect(combinedAdapters.equalityAdapters).toEqual({ [ExampleType]: equalityAdapter, EqualityOnly: equalityAdapter })
	})
	...
	it('supports empty and partial value types', () => {
		...
		expect(extractValueTypeAdapters(valueTypes)).toEqual({ inputValueAdapters: {}, serializationAdapters: {}, equalityAdapters: { EqualityOnly: equalityAdapter } })
	})

	it('allows identical definitions but rejects conflicting duplicate type keys', () => {
		const valueTypes = { [ExampleType]: completeValueType } satisfies ValueTypes
		expect(combineValueTypes(valueTypes, valueTypes)).toEqual(valueTypes)
		expect(() => combineValueTypes(valueTypes, { [ExampleType]: { equality: equalityAdapter } })).toThrow(/duplicate type "Example"/)
	})
	...
	it('rejects malformed registries and value-type definitions', () => {
		...
		expect(() => extractValueTypeAdapters({ Example: { serialization: null } } as never)).toThrow(/complete/)
	})
})
