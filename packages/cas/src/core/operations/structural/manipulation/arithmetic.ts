import { type ExpressionNode, type ExpressionNodeInput, asExpressionNode, Integer, negative, sum, product, fraction } from '../../../construction/index.ts'

import { isSignNode } from '../inspection/index.ts'
...
export const stripSigns = (node: ExpressionNodeInput): ExpressionNode => {
	const expressionNode = asExpressionNode(node)
	return isSignNode(expressionNode) ? stripSigns(expressionNode.node) : expressionNode
}
