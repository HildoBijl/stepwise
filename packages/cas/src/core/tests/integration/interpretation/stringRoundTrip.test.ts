...
describe('toString', () => {
	test.each(parserTestCases)('prints "$str"', ({ str, node, oneWay }) => {
		if (!oneWay) expect(nodeToString(node, inferInterpretationSettingsOptions(node))).toBe(str)
	})
})
...
describe('stringToExpressionNode', () => {
	test.each(parserTestCases)('interprets "$str"', ({ str, node }) => {
		expectNodeToEqual(parseExpressionNode(str, inferInterpretationSettingsOptions(node)), node)
	})
})
...
describe('multi-character variable interpretation', () => {
	test('interprets adjacent letters as separate variables by default', () => {
		expectNodeToEqual(parseExpressionNode('xy', {}), product('x', 'y'))
		expectNodeToEqual(parseExpressionNode('xy2', {}), product('x', 'y', 2))
		expectNodeToEqual(parseExpressionNode('23xy2', {}), product(23, 'x', 'y', 2))
	})
	test('interprets adjacent letters as one variable when enabled', () => {
		expectNodeToEqual(parseExpressionNode('xy', { allowMultiCharacterVariables: true }), variable('xy'))
		expectNodeToEqual(parseExpressionNode('xy2', { allowMultiCharacterVariables: true }), variable('xy2'))
		expectNodeToEqual(parseExpressionNode('23xy2', { allowMultiCharacterVariables: true }), product(23, variable('xy2')))
	})
	test('keeps infinity separate from variables', () => {
		expectNodeToEqual(parseExpressionNode('∞xy', { allowMultiCharacterVariables: true }), product(namedConstants.infinity, variable('xy')))
		expectNodeToEqual(parseExpressionNode('23∞45xy', { allowMultiCharacterVariables: true }), product(23, namedConstants.infinity, 45, variable('xy')))
	})
	...
	test('prints products explicitly when multi-character variables are enabled', () => {
		expect(nodeToString(product('x', 'y'), { allowMultiCharacterVariables: true })).toBe('x*y')
		expect(nodeToString(product(23, 'x', 'y', 2), { allowMultiCharacterVariables: true })).toBe('23x*y*2')
	})
	test('derives interpretation settings from multi-character variables', () => {
		const node = product(variable('xy'), 'z')
		expect(inferInterpretationSettingsOptions(node)).toEqual({ allowMultiCharacterVariables: true })
		expect(nodeToString(node, inferInterpretationSettingsOptions(node))).toBe('xy*z')
	})
	test('does not enable multi-character variables for single-character variables', () => {
		const node = product('x', 'y')
		expect(inferInterpretationSettingsOptions(node)).toEqual({})
		expect(nodeToString(node, inferInterpretationSettingsOptions(node))).toBe('xy')
	})
})
