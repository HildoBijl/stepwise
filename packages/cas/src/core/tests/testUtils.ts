...
export function expectNodeToEqual(result: ExpressionNodeInput, expected: ExpressionNodeInput) {
	...
	if (!areNodesEqual(resultValue, expectedValue, false)) throw new Error(`A node was not what was expected.
...
export function expectSimplifyToGive(input: ExpressionNodeInput, output: ExpressionNodeInput, options: SimplificationOptionsInput, expressionSettings: ExpressionSettingsOptions = {}) {
	...
	if (!areNodesEqual(result, outputValue, false)) throw new Error(`A simplification check did not give the expected result.
