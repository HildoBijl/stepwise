...
import { refrigerantDatasets, getRefrigerantPropertiesFromPressureAndTemperature, getRefrigerantPropertiesFromPressureAndEnthalpy, getSaturatedMixturePropertiesFromTemperature, getSaturatedMixturePropertiesFromPressure } from '@step-wise/physics-data'
	...
	generateParameters() {
		while (true) {
			...
			const parameters = {
				...
				x1: point1.phase === 'mixture' ? point1.vaporFraction!.setDecimals(2).roundToPrecision().setDisplayPower(0) : undefined,
				p1: point1.phase === 'mixture' ? undefined : point1.pressure.setSignificantDigits(2).roundToPrecision(),
				phase2: point2.phase,
				x2: point2.phase === 'mixture' ? point2.vaporFraction!.setDecimals(2).roundToPrecision().setDisplayPower(0) : undefined,
				T2: point2.phase === 'mixture' ? undefined : point2.temperature.setDecimals(0).roundToPrecision(),
			}
			try {
