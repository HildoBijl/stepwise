...
export default buildStepExercise({
	...
	generateParameters() {
		...
		const h1 = getRandomQuantity({ min: 80, max: 110, unit: 'cm', decimals: 0 })
		const h2 = getRandomQuantity({ min: 130, max: 160, unit: 'cm', decimals: 0 })
		const BMI1 = getRandomQuantity({ min: 15, max: 18, unit: 'kg/m^2' })
		const BMI2 = getRandomQuantity({ min: 17, max: 20, unit: 'kg/m^2' })
