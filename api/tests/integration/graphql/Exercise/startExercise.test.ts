...
describe('startExercise', () => {
	...
	it('creates a skill when none exists yet', async () => {
		...
		const { data: { startExercise: exercise }, errors } = await client.graphql({ query: `mutation{startExercise(skillId: "${SAMPLE_SKILL}") {id exerciseId state active startedOn progress lastAction lastActionAt history {mode events {action progress performedAt}}}}` })
		...
		const { data: { skill: skillAfter }, errors: skillAfterErrors } = await client.graphql({ query: `{skill(skillId: "${SAMPLE_SKILL}") {id skillId ... on SkillWithExercises {activeExercise {id exerciseId state active startedOn progress lastAction lastActionAt history {mode events {action progress performedAt}}} exercises {id}}}}` })
