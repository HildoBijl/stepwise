...
describe('submit group action:', () => {
	...
	it('allows submission, resubmission and canceling of actions', async () => {
		...
		const { data: { submitGroupAction: submitExercise }, errors: submitErrors } = await client.graphql({ query: `mutation{submitGroupAction(code: "${GROUP_CODE}", skillId: "${SAMPLE_SKILL}", action: ${stringifyGraphQLInput(action1)}){skillId history{mode events{submissions{userId action}}}}}` })
		expect(submitErrors).toBeUndefined()
		expect(submitExercise.history.mode).toBe('group')
		expect(submitExercise.history.events[0].submissions.length).toStrictEqual(1)
		expect(submitExercise.history.events[0].submissions[0].action).toStrictEqual(action1)
		expect(client.countEvents('GROUP_EXERCISE_UPDATED')).toStrictEqual(2)

		// Submit a second action.
		const action2 = inputAction(28)
		const { data: { submitGroupAction: resubmitExercise }, errors: resubmitErrors } = await client.graphql({ query: `mutation{submitGroupAction(code: "${GROUP_CODE}", skillId: "${SAMPLE_SKILL}", action: ${stringifyGraphQLInput(action2)}){skillId history{mode events{submissions{userId action}}}}}` })
		expect(resubmitErrors).toBeUndefined()
		expect(resubmitExercise.history.events[0].submissions.length).toStrictEqual(1)
		expect(resubmitExercise.history.events[0].submissions[0].action).toStrictEqual(action2)
		expect(client.countEvents('GROUP_EXERCISE_UPDATED')).toStrictEqual(3)

		// Cancel the action.
		const { data: { cancelGroupAction: cancelExercise }, errors: cancelErrors } = await client.graphql({ query: `mutation{cancelGroupAction(code: "${GROUP_CODE}", skillId: "${SAMPLE_SKILL}"){skillId history{mode events{submissions{userId action}}}}}` })
		expect(cancelErrors).toBeUndefined()
		expect(cancelExercise.history.events[0].submissions.length).toStrictEqual(0)
