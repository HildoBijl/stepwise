...
export function createExerciseSampleModel(sequelize: Sequelize): ExerciseSampleModel {
	...
	ExerciseSample.init({
		...
		sequelize,
		modelName: 'exerciseSample',
		indexes: [
			{ fields: ['userSkillId', { name: 'createdAt', order: 'DESC' }, { name: 'id', order: 'DESC' }], name: 'exerciseSamples_userSkillId_createdAt_id' },
			{ fields: ['userSkillId'], name: 'exerciseSamples_userSkillId_active_unique', unique: true, where: { active: true } },
		],
	})
