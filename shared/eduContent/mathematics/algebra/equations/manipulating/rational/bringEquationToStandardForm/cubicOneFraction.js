...
function getSolution(state) {
	...
	const equation = asEquation('ax+b=(cx(x+d))/(x^2+e)').substitute(variables).removeTrivial()[flip ? 'switch' : 'self']()

	// Rewrite the equation in various ways.
	const multiplied = asEquation('(ax+b)(x^2+e) = cx(x+d)').substitute(variables).removeTrivial()[flip ? 'switch' : 'self']()
