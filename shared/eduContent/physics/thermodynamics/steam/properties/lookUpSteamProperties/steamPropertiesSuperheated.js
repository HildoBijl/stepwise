const { randomInteger, tableInterpolate } = require('@step-wise/utils')
...
function generateState() {
	...
	const p = pressureRange[randomInteger(3, Math.min(20, pressureRange.length))] // Limit to a certain part of the table.

	// Extract temperature row.
	const temperatureRange = enthalpy.headers[1]
	const T = temperatureRange[randomInteger(6, Math.min(24, temperatureRange.length))] // Limit to a certain part of the table.
