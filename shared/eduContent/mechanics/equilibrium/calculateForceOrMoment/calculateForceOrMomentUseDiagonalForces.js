const { integerRange, getRandomBoolean, getRandomInteger } = require('@step-wise/utils')
...
function getSolution(state) {
	...
	let decomposedLoadNames = decomposedLoads.map((load, index) => Array.isArray(load) ? [
		{ load: load[0], variable: asExpression(`F_(${pointNames[index]}l)`), point: points[index] },
		{ load: load[1], variable: asExpression(`F_(${pointNames[index]}p)`), point: points[index] },
...
function checkInput(exerciseData, step) {
	switch (step) {
		...
		case 2:
			return performComparison(exerciseData, 'FDl')
		default:
