...
Step-Wise has a large amount of Javascript utility functions: simple functions that do a small task, and can be used anywhere in the code base. These are sorted into various groups. Each utility submodule may use submodules higher up the chain, but not vice versa, to prevent circular references.
...
- [errors](./src/errors/) extends various `Error` classes.
- [dates](./src/dates/) provides extra functionality upon the Javascript `Date` object.
