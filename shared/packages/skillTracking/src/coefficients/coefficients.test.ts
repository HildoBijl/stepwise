...
import { ensureCoefficients, getOrder, normalize } from './fundamentals'
import { getPDF } from './distributions'
import { getExpectedValue, getVariance, getMoment } from './moments'
import { merge, mergeElementwise } from './merging'
import { smoothWithFactor } from './smoothing'
...
describe('Check fundamental functions:', () => {
	describe('ensureCoefficients', () => {
		...
		it('passes on valid coefficient arrays', () => {
			...
			expect(compareNumberArrays(ensureCoefficients(coefficients), coefficients)).toBe(true)
		})
	})
...
describe('Check smoothing functions:', () => {
	describe('smoothWithFactor', () => {
		it('correctly smooths distributions', () => {
			expect(compareNumberArrays(smoothWithFactor([0, 1], 1 / 2), [1 / 6, 1 / 3, 1 / 2])).toBe(true)
			expect(compareNumberArrays(smoothWithFactor([0, 1 / 3, 2 / 3], 3 / 4), [5 / 140, 10 / 140, 15 / 140, 20 / 140, 25 / 140, 30 / 140, 35 / 140])).toBe(true)
		})
	})
})
