...
export function requiresBracketsFor(node: ExpressionNode, level: BracketLevel, index?: number): boolean {
	if (node instanceof ConstantNode) return node.value < 0 && level !== bracketLevels.addition && level !== bracketLevels.multiplication
	...
	if (node instanceof FunctionNode) return level === bracketLevels.powers
	return false
}
