...
import { isSign, isSum, isProduct, isFunctionNode } from '../../operations'
...
export function requiresBracketsFor(node: ExpressionNode, level: BracketLevel, index = 0, length = 0): boolean {
	if (isSign(node)) return (level === bracketLevels.multiplication && index < length - 1) || level === bracketLevels.negation || level === bracketLevels.division || level === bracketLevels.powers
