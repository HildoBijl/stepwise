...
function repeat(times, func) {
	...
	return repeatFromTo(0, times - 1, func)
}
module.exports.repeat = repeat

// repeatFromTo will repeat the given function with indices ranging from min to max (both inclusive). So repeatFromTo(3, 5, print) will print 3, 4 and 5. If min is larger than max, an error will be thrown. Returned is an array of all outcomes. If no function but a value is given, that value is repeated as is.
function repeatFromTo(min, max, func) {
	// Process input.
	min = ensureInt(min)
	max = ensureInt(max)
...
module.exports.repeatFromTo = repeatFromTo
...
function repeatMultidimensional(times, func) {
	...
	return repeatMultidimensionalFromTo(times.map(() => 0), times.map(num => num - 1), func)
}
module.exports.repeatMultidimensional = repeatMultidimensional

// repeatMultidimensionalFromTo will repeat the given function with indices ranging from min to max (both inclusive). So for instance it can be called with min: [3, 8] and max: [5, 9], in which case we get as outcome an array [[f(3,8), f(3,9)], [f(4,8), f(4,9)], [f(5,8), f(5,9)]]
function repeatMultidimensionalFromTo(min, max, func, previousValues = []) {
	...
	return repeatFromTo(minValue, maxValue, value => repeatMultidimensionalFromTo(min.slice(1), max.slice(1), func, [...previousValues, value]))
}
module.exports.repeatMultidimensionalFromTo = repeatMultidimensionalFromTo
