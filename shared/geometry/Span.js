...
const { mergeDefaults } = require('@step-wise/utils')
...
class Span {
	...
	constructor(span) {
		...
		// Process the Span.
		span = mergeDefaults(span, defaultSpan)
		if (!span.end) {
			...
			if (span.vector && !this.vector.equals(span.vector))
				throw new Error(`Invalid Span: the given vector "${span.vector}" is not the difference between the start "${start}" and the end "${end}".`)
		}
	}
...
function ensureSpan(span, dimension) {
...
module.exports.ensureSpan = ensureSpan