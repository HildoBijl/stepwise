...
function getCurrentInputSolutionAndComparison(currParameter, input, solution, comparison, singleParameterCase) {
	// Extract the current input value.
	const currInput = getPropertyOrDefault(input, currParameter, false, singleParameterCase, true, `Field check error: could not find an input for field "${currParameter}". Make sure that there is an input field named "${currParameter}".`)

	// Extract the current solution
	const currSolution = getPropertyOrDefault(solution, currParameter, false, singleParameterCase, true, `Field check error: could not find a solution answer for field "${currParameter}". Make sure it is exported from the getSolution function. Either the getSolution function should export an object { param1: ..., param2: ... } or, in case the exercise has only a single answer, the getSolution function can also export this single answer. In this case the "parameters" argument may not be an array.`)
	...
	const currComparison = getPropertyOrDefault(comparison, currParameter, true, singleParameterCase, true, `Field comparison error: could not find a comparison function or comparison options for field "${currParameter}". Make sure that the comparison object has a parameter with this name, or otherwise a parameter "default". (This could even be an empty object if default comparison options are used.)`)
...
function performIndividualListComparison(inputParameter, solutionParameter, input, solution, comparison) {
	const { currInput } = getCurrentInputSolutionAndComparison(inputParameter, input, solution, comparison, false)
	const { currSolution, currComparison } = getCurrentInputSolutionAndComparison(solutionParameter, input, solution, comparison, true)
	return performIndividualComparison(solutionParameter, currInput, currSolution, currComparison, solution)
}
