...
export function Translation({ path, entry, children, extendEntry }) {
	...
	try {
		...
		if (isLocalhost()) {
			console.error(`Translation failure: tried to translate an element with path "${path === undefined ? '[inherited]' : path}" and entry "${entry}" but failed to do so. Probably the translation text does not have the right format. The texts are as follows.\nTranslation text: ${text}\nOriginal text: ${fallbackText}`)
			console.error(error)
		}
		return applyNoTranslation(children)
	}
