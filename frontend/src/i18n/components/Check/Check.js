...
export function Check({ children, value }) {
	// Check the input.
	if (value === undefined)
		throw new Error(`Check error: no value has been passed to the Check. (It does not accept undefined.) Make sure a value that is non-undefined is inserted. (If needed, you can use value={!!value} to guarantee a boolean value.)`)
