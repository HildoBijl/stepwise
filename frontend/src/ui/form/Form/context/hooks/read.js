...
export function useInputObject(ids, useFI = false) {
	...
	const { input, getAllInputFI, getAllInputFO, getInputFI, getInputFO } = useFormData() // Input dependency is needed to update on input changes. The get-functions themselves are stable.
...
export function useInput(ids, useFI = false) {
	...
	return Array.isArray(ids) ? ids.map(id => inputObject[id]) : inputObject
}
