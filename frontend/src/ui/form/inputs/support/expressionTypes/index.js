 ...
 * - keyPressToData(keyInfo, data, settings, charElements, topParentData, contentsElement, cursorElement): takes a keyInfo event, a data object and some reference elements. It uses this to determine a new data object: how the data (value and cursor) change depending on the given key press.
 ...
 * - cleanUp(data, settings): takes an element and cleans it up, removing unnecessary elements and such. While doing so, the respective cursor is kept in the same place. If no cursor is provided, no cursor is returned either. Generally, the clean-up function is called by the input field after every data change. The settings are general input field settings, which give data on what kind of input is allowed.
 * 
