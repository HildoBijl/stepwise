...
export const defaultFieldRegistrationOptions = {
	...
	keyboard: undefined, // A keyboard object describing all the details of what keyboard should be shown when this field is active. It is a plain object of the form { keyFunction: keyInfo => (...change state...), settings: {...} }. When undefined, no keyboard is shown. For details, see the Keyboard component and its handlers.
}
export function useFieldRegistration(options) {
	// Process input.
	const { id, element, apply, useTabbing, manualIndex, autofocus, focusOnClick, focusRefOnActive, keyboard } = mergeDefaults(options, defaultFieldRegistrationOptions)
	if (apply && !element)
