...
export default function Component() {
	return <>
		...
		<Par>Stel je de lucht in de zuiger van een benzinemotor voor. Als deze lucht onder hoge druk staat, dan kan de lucht de zuiger wegduwen. Zo verricht de lucht arbeid: het duwt iets weg! Immers, arbeid is kracht maal afstand: <M>W = Fs.</M> Maar hoe kunnen we hiermee rekenen?</Par>
		...
		<Par>De algemene formule voor arbeid, bij een polytroop proces, is <BM>W_(1-2) = -\frac(1)(n-1) \left(p_2V_2 - p_1V_1\right).</BM> Als je een proces weet, en daarmee ook de waarde van <M>n,</M> dan kun je deze formule direct toepassen.</Par>
		<Par>Er is één uitzondering bij het bovenstaande: als <M>n = 1</M> (dus bij een isotherm proces) werkt de formule niet. Het gaat mis bij de breuk: we mogen niet door nul delen. Voor isotherme processen is er dus een andere formule, en wel <BM>W_(1-2) = pV \ln\left(\frac(V_2)(V_1)\right).</BM></Par>
		...
		<List items={[
			<>Voor isobare processen geldt <BMList><BMPart>W_(1-2) = p\left(V_2 - V_1\right),</BMPart><BMPart>W_(1-2) = mR_s\left(T_2 - T_1\right).</BMPart></BMList></>,
			<>Voor isochore processen geldt altijd <M>W_(1-2) = 0.</M> Immers, het volume blijft constant, dus er wordt niets weggeduwd. Er wordt dus ook geen arbeid verricht.</>,
			<>Voor isotherme processen geldt <BMList><BMPart>W_(1-2) = pV \ln\left(\frac(V_2)(V_1)\right),</BMPart><BMPart>W_(1-2) = mR_sT \ln\left(\frac(V_2)(V_1)\right).</BMPart></BMList></>,
			<>Voor isentrope processen geldt <BMList><BMPart>W_(1-2) = -\frac(1)(k-1) \left(p_2V_2 - p_1V_1\right),</BMPart><BMPart>W_(1-2) = -\frac(mR_s)(k-1)\left(T_2 - T_1\right).</BMPart></BMList></>,
			<>Voor polytrope processen, met een algemene <M>n,</M> geldt <BMList><BMPart>W_(1-2) = -\frac(1)(n-1) \left(p_2V_2 - p_1V_1\right),</BMPart><BMPart>W_(1-2) = -\frac(mR_s)(n-1)\left(T_2 - T_1\right).</BMPart></BMList></>,
		]} />
		...
		<Par>Bij het rekenen met arbeid is het belangrijk om de tekenconventie in gedachten te houden. We nemen (als afspraak) altijd aan dat het gas arbeid verricht op de omgeving. Dit betekent dat het gas expandeert (toeneemt in volume) en dat de pijl in het <M>p</M>-<M>V</M>-diagram dus van links naar rechts gaat. Dit is positieve arbeid. Denk aan de lucht in de benzinemotor die de zuiger wegduwt.</Par>
		<Par>In de omgekeerde situatie oefenen wij vanuit de omgeving juist arbeid uit op het gas. Denk aan het indrukken van een fietspomp. In dit geval wordt het gas gecomprimeerd (afname in volume) en gaat de pijl in het <M>p</M>-<M>V</M>-diagram juist van rechts naar links. Dit is negatieve arbeid.</Par>
		...
		<Par>Na arbeid bekijken we warmte. Stel je de lucht in de zuiger van een benzinemotor voor, zowel vlak voordat het brandstof ontbrand is als vlak nadat het brandstof ontbrand is. Het verschil tussen deze twee situaties is: er is warmte toegevoerd, via het ontbranden van brandstof. Maar hoeveel?</Par>

		<SubHead>De soortelijke warmte</SubHead>
		<Par>De soortelijke warmte <M>c</M> is, per definitie, de hoeveelheid energie (in Joule) die nodig is om één kilogram van een stof één graad Celsius (of één Kelvin) op te warmen. Voor vaste en vloeibare stoffen is deze waarde ongeveer constant, waardoor <BM>Q = mc \, \Delta T.</BM> Voor gassen is dit echter niet zo: de waarde van <M>c</M> hangt óók af van het soort proces! Laten we dit voor de ons bekende processen (isobaar, isochoor, isotherm, isentroop en (algemeen) polytroop) nader bekijken.</Par>
		<List items={[
			<>Bij een isobaar proces is de soortelijke warmte (per definitie) gelijk aan de "soortelijke warmte bij constante druk" <M>c_p.</M> Dit is een gaseigenschap die je op kan zoeken. Er geldt dus <M>c = c_p.</M></>,
			<>Bij een isochoor proces is de soortelijke warmte (per definitie) gelijk aan de "soortelijke warmte bij constant volume" <M>c_v.</M> Dit is een gaseigenschap die je op kan zoeken. Er geldt dus <M>c = c_v.</M></>,
			<>Bij een isotherm proces neemt de temperatuur nooit toe. Er kan wel warmteuitwisseling zijn, maar dan op zo'n manier dat de temperatuur niet verandert. In dit geval geldt <M>c = \infty.</M></>,
			<>Bij een isentroop proces is er nooit warmteuitwisseling. De temperatuur kan wel veranderen. In dit geval geldt <M>c = 0.</M></>,
			<>Bij een polytroop proces met procescoëfficiënt <M>n</M> kun je de soortelijke warmte <M>c</M> berekenen. Dit gaat via de formule <BM>c = c_v - \frac(R_s)(n-1).</BM></>,
		]} />
		...
		<List items={[
			<>Voor isobare processen geldt <BMList><BMPart>Q_(1-2) = \frac(k)(k-1) p\left(V_2 - V_1\right),</BMPart><BMPart>Q_(1-2) = mc_p \left(T_2 - T_1\right).</BMPart></BMList></>,
			<>Voor isochore processen geldt <BMList><BMPart>Q_(1-2) = \frac(1)(k-1) V\left(p_2 - p_1\right),</BMPart><BMPart>Q_(1-2) = mc_v \left(T_2 - T_1\right).</BMPart></BMList></>,
			<>Voor isotherme processen geldt <BMList><BMPart>Q_(1-2) = pV \ln\left(\frac(V_2)(V_1)\right),</BMPart><BMPart>Q_(1-2) = mR_sT \ln\left(\frac(V_2)(V_1)\right).</BMPart></BMList></>,
			<>Voor isentrope processen geldt altijd <M>Q_(1-2) = 0.</M> Immers, een isentroop proces is per definitie een proces zonder warmteuitwisseling.</>,
			<>Voor polytrope processen, met een algemene <M>n,</M> bereken je eerste <M>c = c_v - \frac(R_s)(n-1).</M> Vervolgens geldt <BMList><BMPart>Q_(1-2) = \frac(c)(R_s) \left(p_2V_2 - p_1V_1\right),</BMPart><BMPart>Q_(1-2) = mc \left(T_2 - T_1\right).</BMPart></BMList></>,
		]} />
