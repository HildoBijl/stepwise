...
const steps = [
	{
		Problem: (state) => {
			...
			return <>
				<Par>Splits de breuk simpelweg op in twee losse breuken met een {plus ? 'plus' : 'min'}teken ertussen. (Pas nog geen simplificaties toe.)</Par>
				<InputSpace>
					<Par>
						<ExpressionInput id="split" prelabel={<M>{expression}=</M>} label="Vul hier het resultaat in" size="l" settings={basicMath} validate={validWithVariables(Object.values(variables))} />
					</Par>
				</InputSpace>
			</>
		},
	...
	},
	{
		...
		Solution: (state) => {
			...
			return <Par>Bij de eerste breuk kunnen we boven en onder <M>{leftExpression.numerator}</M> weg delen. Zo vinden we <BM>{leftExpression} = {leftAns}.</BM> Voor de tweede breuk delen we boven en onder <M>{rightExpression.numerator}</M> weg. Hiermee krijgen we <BM>{rightExpression} = {rightAns}.</BM></Par>
		},
	},
	{
		Problem: (state) => {
...
function getFeedback(exerciseData) {
	...
	// Determine feedback.
	return getInputFieldFeedback(['ans', 'denominator', 'leftAns', 'rightAns'], exerciseData, [ansChecks, splitChecks, fractionChecks, fractionChecks].map(feedbackChecks => ({ feedbackChecks })))
}