...
const steps = [
	...
	},
	{
		...
		Solution: (state) => {
			...
			if (choice === undefined || choice === 0)
				return <Par>We moeten alleen nog de temperatuur <M>T</M> weten. Deze vinden we via de gaswet, toegepast op punt 2. Oftewel, <BM>p_2 V_2 = m R_s T_2.</BM> Dit oplossen voor <M>T_2</M> geeft <BM>T_2 = \frac(p_2V_2)(m R_s) = \frac({p2.float} \cdot {V2.float})({m.float} \cdot {Rs.float}) = {T2}.</BM> Dit is een stuk kouder dan in de verbrandingskamer, wat logisch is: de lucht expandeert in de turbine, en bij expanderen daalt de temperatuur altijd. Hoewel een temperatuur van <M>T_2 = {T2.setUnit('dC').useDecimals(0)}</M> natuurlijk nog steeds best warm is.</Par>
