		...
		<InputSpace>
			...
			{numSolutions ? <Par>
				{numberArray(1, numSolutions).map(index =>
					<ExpressionInput key={index} id={`x${index}`} prelabel={<M>{x}_{index}=</M>} label={`Vul hier antwoord ${index} in`} size="s" settings={basicMathAndPowers} validate={validAndNumeric} persistent={true} />
				)}
			</Par> : null}
		</InputSpace>
	</>
	...
	},
	{
		Problem: () => {
			...
			return <>
				...
				<InputSpace>
					<Par>
						{numberArray(1, 2).map(index => <ExpressionInput key={index} id={`x${index}`} prelabel={<M>{x}_{index}=</M>} label={`Vul hier antwoord ${index} in`} size="s" settings={basicMathAndPowers} validate={validAndNumeric} persistent={true} />
						)}
					</Par>
				</InputSpace>
			</>
