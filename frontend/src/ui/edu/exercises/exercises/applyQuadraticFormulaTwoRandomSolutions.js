		...
		<InputSpace>
			...
			{numSolutions ? <Par>
				{numberArray(1, numSolutions).map(index =>
					<ExpressionInput key={index} id={`x${index}`} prelabel={<M>{x}_{index}=</M>} label={`Vul hier antwoord ${index} in`} size="s" settings={basicMathAndPowers} validate={numeric} persistent={true} />
				)}
			</Par> : null}
		</InputSpace>
	</>
