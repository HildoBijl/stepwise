...
const steps = [
	...
	},
	{
		...
		Solution: (state) => {
			const { shared: { getCorrect } } = useExerciseData()
			const { m, cv, T1, T2, T3, T4, Q12, W12, Q23, W23, Q34, W34, Q41, W41, Wn } = getCorrect(state)
			return <>
				<Par>
	...
	},
	{
		...
		Solution: (state) => {
			const { shared: { getCorrect } } = useExerciseData()
			const { Wn, Qin, eta } = getCorrect(state)
			return <Par>
...
const getFeedback = (exerciseData) => {
	...
	return {
		...getDefaultFeedback(['p1', 'V1', 'T1', 'p2', 'V2', 'T2', 'p3', 'V3', 'T3', 'Q12', 'W12', 'Q23', 'W23', 'Q34', 'W34', 'Q41', 'W41', 'eta'], exerciseData),
