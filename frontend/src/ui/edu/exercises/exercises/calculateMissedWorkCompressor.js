...
const steps = [
	...
	},
	{
		...
		Solution: () => {
			const { p1, p2, T1, T2, dsIn, cp, Rs } = useCorrect()
			return <>
	...
	},
	{
		...
		Solution: () => {
			const { n, T1, T2, cv, Rs, c, q } = useCorrect()
			return <>
	...
	},
	{
		...
		Solution: () => {
			const { dsIn, dsOut, ds } = useCorrect()
			return <>
...
const getFeedback = (exerciseData) => {
	return getDefaultFeedback(['T2', 'dsIn', 'q', 'dsOut', 'ds', 'wm'], exerciseData)
}
