...
const Problem = (state) => {
	...
	return <>
		...
		<Table colHeads={['Druk', 'Specifiek volume', 'Temperatuur']} rowHeads={['Punt 1', 'Punt 2', 'Punt 3']} fields={[[<M>p_1 = {p1.setUnit('bar')}</M>, <M>v_1 = {v1}</M>, <M>T_1 = {T1}</M>], [<M>p_2 = {p2.setUnit('bar')}</M>, <M>v_2 = {v2}</M>, <M>T_2 = {T2}</M>], [<M>p_3 = {p3.setUnit('bar')}</M>, <M>v_3 = {v3}</M>, <M>T_3 = {T3}</M>]]} style={{ minWidth: '500px', maxWidth: '540px' }} />
...
const steps = [
	{
		...
		Solution: (state) => {
			...
			const { q12, wt12 } = getCorrect(state)
	...
	{
		Problem: () => <>
			<Par>Tenslotte is er stap 3-1. Hier wordt het gas <strong>isobaar</strong> opgewarmd. Bereken met behulp van de gegeven waarden de toegevoerde warmte en de door het gas geleverde technische arbeid.</Par>
			<InputSpace>
				<Par>
		...
		Solution: (state) => {
			...
			const { cp, q12, wt12, q23, wt23, q31, wt31, qn, wn } = getCorrect(state)
...
const getFeedback = (exerciseData) => {
	return getDefaultFeedback(['q12', 'wt12', 'q23', 'wt23', 'q31', 'wt31'], exerciseData)
}
