...
const Problem = (state) => {
	...
	return <>
		...
		<InputSpace>
			...
			{numSolutions ? <Par>
				{numberArray(1, numSolutions).map(index => <ExpressionInput key={index} id={`γ${numSolutions > 1 ? index : ''}`} prelabel={<M>{γ}{numSolutions > 1 ? `_${index}` : ''}=</M>} size="m" settings={basicTrigonometryInDegrees} validate={validAndNumeric} persistent={true} />)}
			</Par> : null}
		</InputSpace>
	</>
}
	...
	},
	{
		Problem: (state) => {
			...
			return <>
				...
				<InputSpace>
					<EquationInput id="equation" settings={basicTrigonometryInDegrees} validate={validWithVariables(γ)} />
				</InputSpace>
			</>
		},
		Solution: (state) => {
	...
	},
	{
		Problem: (state) => {
			...
			return <>
				...
				<InputSpace>
					<Par>
						<ExpressionInput id="γ1" prelabel={<M>{γ}_1=</M>} size="m" settings={basicTrigonometryInDegrees} validate={validAndNumeric} />
						<ExpressionInput id="γ2" prelabel={<M>{γ}_2=</M>} size="m" settings={basicTrigonometryInDegrees} validate={validAndNumeric} />
					</Par>
				</InputSpace>
			</>
		},
