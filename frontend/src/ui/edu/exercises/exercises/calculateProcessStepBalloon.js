...
const steps = [
	{
		...
		Solution: (state) => {
			...
			return <>
				...
				<Par>De gaswet zegt dat <BM>pV = mR_sT.</BM> Dit toepassen op punt 1 en oplossen voor <M>p_1</M> geeft <BM>p_1 = \frac(mR_sT_1)(V_1) = \frac({ms.float} \cdot {Rs.float} \cdot {T1s.float})({V1s.float}) = {p1}.</BM> Onafgerond was dit antwoord <M>p_1 = {p1.useUnit('bar').useDecimals(2)}</M>, wat net iets hoger is dan de atmosferische druk. Dit is logisch: een ballon trekt wel een klein beetje extra op de lucht die erin zit, waardoor de druk ietsje hoger is dan atmosferische druk, maar een ballon is relatief flexibel, dus deze overdruk is minimaal.</Par>
			</>
		},
	},
	{
		...
		Solution: (state) => {
			...
			return <Par>Op het moment is de druk in de ballon <M>{p1.useUnit('bar').useDecimals(2)}</M>. Dit is gelijk aan de atmosferische druk plus nog een klein beetje extra druk die, vanwege de elasticiteit van de ballon, toegevoegd wordt. De ballon "trekt aan" waardoor de lucht nog net ietsje meer samengedrukt wordt. Omdat de elasticiteit van de ballon bij benadering constant blijft, zal deze "extra druk" ook hetzelfde blijven. Kortom: de druk blijft gelijk. Het is (bij benadering) een isobaar proces, waardoor <M>p_2 = p_1</M>.</Par>
		},
	},
	{
		Problem: () => <>
		...
		Solution: (state) => {
			...
			return <Par>We weten inmiddels dat <BM>p_2 = p_1 = {p2}.</BM> Ook is de eindtemperatuur <M>T_2</M> bekend. In standaard eenheden is deze <BM>T_2 = {T2.float} + {TConversion.float} = {T2s}.</BM> Alleen <M>V_2</M> is nog onbekend. Deze kunnen we vinden via de gaswet <BM>pV = mR_sT.</BM> Als we deze oplossen voor <M>V_2</M> vinden we <BM>V_2 = \frac(mR_sT_2)(p_2) = \frac({m.float} \cdot {Rs.float} \cdot {T2s.float})({p2.float}) = {V2}.</BM> Dit komt neer op <M>{V2.useUnit('l')}</M> wat een beetje groter is dan het beginvolume van <M>{V1}</M>. Dit klinkt logisch: de ballon zet immers uit.</Par>
		},
	},
]
