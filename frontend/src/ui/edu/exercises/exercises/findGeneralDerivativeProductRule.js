	...
	},
	{
		...
		Solution: () => {
			...
			return <Par>Als we de functie <M>h\left({x}\right)</M> opsplitsen bij de vermenigvuldiging, dan zien we direct dat
				...
				aan de gestelde voorwaarden voldoen.{Math.abs(c) === 1 ? '' : <> Het maakt hierbij overigens niet uit of de constante <M>{c}</M> bij <M>f</M> of bij <M>g</M> gevoegd wordt.</>}</Par>
		},
	},
	{
		Problem: () => {
...
function getFeedback(exerciseData) {
	...
	const funcFeedback = {}
	if (input.f && input.g) {
		const isFCorrect = shared.checkF(input.f, solution)
		const isGCorrect = shared.checkF(input.g, solution)
		const isEquivalent = input.f && input.g && equivalent(input.f.multiply(input.g), solution.h)
		...
		if (areFAndGCorrect) {
			...
			funcFeedback.f.text = <>Dit is wat flauw. Het klopt wel dat <M>h\left({x}\right) = f\left({x}\right) g\left({x}\right)</M> maar je maakt het probleem er niet makkelijker mee.</>
			...
			funcFeedback.g.text = <>Dit is ook een mogelijke functie <M>g\left({x}\right).</M> Maar nog steeds geldt niet dat <M>h\left({x}\right) = f\left({x}\right) g\left({x}\right).</M> Kijk hier eens naar.</>
		} else if (isFCorrect) {
			funcFeedback.f.correct = true
			funcFeedback.f.text = <>Dit is een mogelijke functie <M>f\left({x}\right).</M></>
			funcFeedback.g.text = <>Hier gaat het helaas fout. Dit is geen correcte functie <M>g\left({x}\right).</M> Kijk goed of wel geldt dat <M>h\left({x}\right) = f\left({x}\right) g\left({x}\right).</M></>
		} else if (isGCorrect) {
			funcFeedback.g.correct = true
			funcFeedback.f.text = <>Dit is helaas geen mogelijke functie <M>f\left({x}\right).</M> Zorg wel dat <M>h\left({x}\right) = f\left({x}\right) g\left({x}\right)</M> geldt.</>
