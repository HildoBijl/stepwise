...
function getFeedback(exerciseData) {
	// Define ans checks.
	const outsideBracketsForm = (input, correct, { variables }) => !(input.isType(Product) && input.terms.length === 2 && input.terms.some(term => onlyOrderChanges(variables.x, term)) && input.terms.some(term => term.isType(Sum))) && <>Je antwoord moet van de vorm <M>{variables.x} \cdot \left(\ldots\right)</M> zijn.</>
	...
	// Define setup checks.
	const setupForm = (input, correct, { variables }) => !(input.isType(Product) && input.terms.length === 2 && input.terms.some(term => onlyOrderChanges(variables.x, term)) && input.terms.some(term => term.isType(Fraction))) && <>Je antwoord moet van de vorm <M>{variables.x} \cdot \frac(\left[\ldots\right])({variables.x})</M> zijn.</>

	const fractionForm = (input, correct, { variables }) => !(input.isType(Product) && input.terms.length === 2 && input.terms.some(term => term.isType(Fraction) && onlyOrderChanges(variables.x, term.denominator))) && <>Je antwoord moet van de vorm <M>{variables.x} \cdot \frac(\left[\ldots\right])({variables.x})</M> zijn. Heb je wel een breuk met noemer <M>{variables.x}</M> ingevoerd?</>

	const correctNumerator = (input, correct, { expression }) => !(input.isType(Product) && input.terms.length === 2 && input.terms.some(term => term.isType(Fraction) && onlyOrderChanges(expression, term.numerator))) && <>Zorg dat je bovenin de breuk letterlijk de uitdrukking <M>{expression}</M> invoert.</>
