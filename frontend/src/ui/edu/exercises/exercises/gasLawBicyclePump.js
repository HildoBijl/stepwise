...
const steps = [
	...
	},
	{
		...
		Solution: (state) => {
			...
			return <>
				<Par>Als eerste zetten we de eigenschappen van het eindpunt in standaard eenheden: <BM>p_2 = {p2},</BM><BM>V_2 = {V2}.</BM> Vervolgens passen we de gaswet <M>pV = mR_sT</M> toe op punt 2: de fietspomp met de hendel ingedrukt. Om deze wet op te lossen voor de temperatuur <M>T</M> delen we beide kanten van de vergelijking door <M>mR_s.</M> Zo vinden we <BM>T_2 = \frac(p_2V_2)(mR_s) = \frac({p2.float} \cdot {V2.float})({m.float} \cdot {Rs.float}) = {T2}.</BM>Je kunt dit eventueel nog omrekenen naar <M>{T2.setUnit('dC').useDecimals(0)}.</M> Dit is een stuk warmer dan de begintemperatuur van <M>{state.T1}.</M> Zo zien we dat lucht bij compressie best veel kan opwarmen.</Par>
