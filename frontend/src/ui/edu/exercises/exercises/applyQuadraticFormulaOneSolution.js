	...
	},
	{
		Problem: (state) => {
			...
			return <>
				...
				<InputSpace>
					<Par>
						<ExpressionInput id="x1" prelabel={<M>{x}_1=</M>} label="Vul hier je antwoord in" size="s" settings={basicMathAndPowers} validate={validAndNumeric} persistent={true} />
					</Par>
				</InputSpace>
			</>
		},
		Solution: (state) => {
			const { x, a, b, D, x1, equationInFactors } = useSolution(state)
			return <Par>
