...
import { useExerciseData, StepExercise, useSolution, equationChecks, getInputFieldFeedback, getInputFieldListFeedback, getMCFeedback } from 'ui/eduTools'
...
const Problem = (state) => {
	...
	return <>
		...
		<InputSpace>
			...
			{numSolutions ? <Par>
				{numberArray(1, numSolutions).map(index => <ExpressionInput key={index} id={`b${numSolutions > 1 ? index : ''}`} prelabel={<M>{b}{numSolutions > 1 ? `_${index}` : ''}=</M>} size="m" settings={ExpressionInput.settings.basicTrigonometryInDegrees} validate={ExpressionInput.validation.numeric} persistent={true} />)}
			</Par> : null}
		</InputSpace>
	</>
}
