...
const steps = [
	...
	},
	{
		Problem: ({ type, year1, year2, pop1, pop2, pop }) => type === 1 ? <>
			<Par>Uitgaande van dat dit deel van de tijd gepasseerd is, wat is de bevolking (afgerond) op dit moment? Merk op dat de bevolking groeit van <M>{pop1}</M> tot <M>{pop2}.</M></Par>
			<InputSpace>
				<Par>
					<IntegerInput id="pop" prelabel={<M>(\rm Aantal) =</M>} label="Aantal mensen" size="s" />
				</Par>
				...
				<Par>Uitgaande van dat dit deel van de tijd gepasseerd is, wat is het jaartal op dit moment? Merk op dat de tijdsperiode van <M>{year1}</M> tot <M>{year2}</M> is.</Par>
				<InputSpace>
					<Par>
						<IntegerInput id="year" prelabel={<M>(\rm Jaar) =</M>} label={`Jaar met ${pop} mensen`} size="s" />
					</Par>
