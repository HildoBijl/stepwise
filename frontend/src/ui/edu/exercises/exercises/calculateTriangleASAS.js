...
const Problem = (state) => {
	...
	return <>
		...
		<ExerciseFigure />
		<InputSpace>
			<ExpressionInput id="a" prelabel={<M>{a}=</M>} size="s" settings={basicTrigonometryInDegrees} validate={validAndNumeric} />
		</InputSpace>
	</>
}
...
const steps = [
	{
		Problem: () => {
			return <>
				...
				<InputSpace>
					<ExpressionInput id="γ" prelabel={<M>γ=</M>} size="s" settings={basicTrigonometryInDegrees} validate={validAndNumeric} />
				</InputSpace>
			</>
		},
		Solution: () => {
	...
	},
	{
		Problem: (state) => {
			const { a } = state
			return <>
				<Par>Bepaal hoeveel oplossingen (passend bij de driehoek) deze vergelijking heeft.</Par>
				<InputSpace>
					<MultipleChoice id="numSolutions" choices={[
						<>Er zijn geen oplossingen voor <M>{a}</M>.</>,
						<>Er is <M>1</M> oplossing voor <M>{a}</M>.</>,
						<>Er zijn <M>2</M> oplossingen voor <M>{a}</M>.</>,
					]} />
				</InputSpace>
			</>
		},
	...
	},
	{
		...
		Solution: (state) => {
			...
			return <Par>Om <M>{state.a}</M> vermenigvuldigen we beide kanten van de vergelijking met <M>\sin\left({α}\right).</M> Zo vinden we <BM>{state.a} = {aRaw}.</BM> Hiermee is de gevraagde zijde berekend.</Par>
		},
	},
]
