...
const steps = [
	{
		...
		Solution: (state) => {
			...
			const { Rs, k, p1, v1, T1, p2, v2, T2, p3, v3, T3, p4, v4, T4 } = getCorrect(state)
			return <>
	...
	},
	{
		...
		Solution: (state) => {
			const { shared: { getCorrect } } = useExerciseData()
			const { cp, T1, T2, T3, T4, q12, wt12, q23, wt23, q34, wt34, q41, wt41, qn, wn } = getCorrect(state)
			return <>
	...
	},
	{
		...
		Solution: (state) => {
			const { shared: { getCorrect } } = useExerciseData()
			const { mdot, wn, qin, qout, epsilon, COP, Ph } = getCorrect(state)
			return <>
...
const getFeedback = (exerciseData) => {
	return getDefaultFeedback(['p1', 'v1', 'T1', 'p2', 'v2', 'T2', 'p3', 'v3', 'T3', 'p4', 'v4', 'T4', 'q12', 'wt12', 'q23', 'wt23', 'q34', 'wt34', 'q41', 'wt41', 'epsilon', 'COP', 'Ph'], exerciseData)
}
