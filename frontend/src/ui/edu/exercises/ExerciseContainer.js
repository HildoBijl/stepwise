...
export default function ExerciseContainer({ exercise, groupExercise, submitting, submitAction, cancelAction, resolveEvent, startNewExercise }) {
	...
	const reload = () => {
		setLoading(true)
		Promise.all([
			import(/* webpackChunkName: "front-end-exercises-2" */ `./exercises/${exerciseId}`),
			import(/* webpackChunkName: "shared-exercises-2" */ `step-wise/edu/exercises/exercises/${exerciseId}`),
