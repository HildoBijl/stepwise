...
const steps = [
	...
	},
	{
		...
		Solution: ({ x, fDerivative, gDerivative }) => {
			return <Par>Voor de afgeleide van <M>f\left({x}\right)</M> hebben we de productregel nodig. Via de gebruikelijke stappen vinden we zo
...
function getFeedback(exerciseData) {
	...
	const fDerivativeIncorrect = (input, correct, solution, isCorrect) => !isCorrect && <>Deze klopt niet. Heb je de productregel wel goed toegepast?</>
	...
	return {
		...getMCFeedback('method', exerciseData, {
			text: [
				<>Bijna! Er staat technisch wel een vermenigvuldiging van "iets met <M>{x}</M>" maal "iets met <M>{x}</M>" in de functie <M>h\left({x}\right).</M> Dit is echter niet de <em>laatste</em> operatie die je uitvoert.</>,
