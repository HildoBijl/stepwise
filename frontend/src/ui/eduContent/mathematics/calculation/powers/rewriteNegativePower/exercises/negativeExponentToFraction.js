...
function Problem() {
	...
	return <Translation>
		<Par>Rewrite the number <M>{expression}</M> to a form without a negative exponent.</Par>
		<InputSpace>
			<Par>
...
function Solution({ expression, ans, simplified }) {
	return <Translation><Par>According to the rule <M>a^(-b) = \frac(1)(a^b)</M> we may directly rewrite the number to <BM>{expression} = {ans}.</BM> Optionally, we may simplify this further to <M>{simplified}</M>.</Par></Translation>
}
