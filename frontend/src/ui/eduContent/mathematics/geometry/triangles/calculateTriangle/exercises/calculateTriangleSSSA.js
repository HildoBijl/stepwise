...
const Problem = ({ α, a, b, c }) => {
	...
	return <>
		...
		<InputSpace>
			...
			{numSolutions ? <Par>
				{integerRange(1, numSolutions).map(index => <ExpressionInput key={index} id={`α${numSolutions > 1 ? index : ''}`} prelabel={<M>{α}{numSolutions > 1 ? `_${index}` : ''}=</M>} size="m" settings={ExpressionInput.settings.basicTrigonometryInDegrees} validate={ExpressionInput.validation.numeric} persistent={true} />)}
			</Par> : null}
		</InputSpace>
	</>
}
