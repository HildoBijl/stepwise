	...
	},
	{
		Problem: ({ β, γ }) => {
			return <>
				...
				<InputSpace>
					<Par>
						<ExpressionInput id="β1" prelabel={<M>{β}_1=</M>} size="m" settings={ExpressionInput.settings.basicTrigonometryInDegrees} validate={ExpressionInput.validation.numeric} />
						<ExpressionInput id="β2" prelabel={<M>{β}_2=</M>} size="m" settings={ExpressionInput.settings.basicTrigonometryInDegrees} validate={ExpressionInput.validation.numeric} />
					</Par>
				</InputSpace>
			</>
		},
		Solution: ({ α, β, γ, γ1, γ2, β1, β2 }) => {
			return <>
				<Par>De som van de hoeken van een driehoek is altijd <M>180^\circ.</M> Hiermee kunnen we direct <M>{β}_1</M> en <M>{β}_2</M> vinden als
					<BMList>
						<BMPart>{β}_1 = 180 - {α} - {γ}_1 = 180 - {α} - {γ1} = {asExpression(180, undefined, { angleUnit: 'degrees' }).subtract(α).combine()} - {γ1},</BMPart>
						<BMPart>{β}_2 = 180 - {α} - {γ}_2 = 180 - {α} - \left({γ2}\right) = {γ1} - {α}.</BMPart>
					</BMList>
					Eventueel kunnen we deze twee oplossingen nog als getal uitrekenen. Zo krijgen we <M>{β}_1 \approx {new PrecisionNumber(roundToDigits(β1.toNumber(), 3))}^\circ</M> en <M>{β}_2 \approx {new PrecisionNumber(roundToDigits(β2.toNumber(), 3))}^\circ.</M> De bij de vraag getekende figuur is gebaseerd op <M>{β}_1.</M> We kunnen met <M>{β}_2</M> ook nog een driehoek tekenen. We krijgen dan de onderstaande afbeelding.</Par>
