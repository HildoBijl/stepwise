...
const Problem = ({ α, a, b, c }) => {
	...
	return <>
		...
		<InputSpace>
			...
			{numSolutions ? <Par>
				{integerRange(1, numSolutions).map(index => <ExpressionInput key={index} id={`b${numSolutions > 1 ? index : ''}`} prelabel={<M>{b}{numSolutions > 1 ? `_${index}` : ''}=</M>} size="m" settings={ExpressionInput.settings.basicTrigonometryInDegrees} validate={ExpressionInput.validation.numeric} persistent={true} />)}
			</Par> : null}
		</InputSpace>
	</>
}
	...
	},
	{
		Problem: ({ α, b }) => {
			return <>
				...
				<InputSpace>
					<EquationInput id="equation" settings={EquationInput.settings.basicTrigonometryInDegrees} validate={EquationInput.validation.validWithVariables(b)} />
				</InputSpace>
			</>
		},
	...
	},
	{
		Problem: ({ b }) => {
			return <>
				...
				<InputSpace>
					<Par>
						<ExpressionInput id="b1" prelabel={<M>{b}_1=</M>} size="m" settings={ExpressionInput.settings.basicTrigonometryInDegrees} validate={ExpressionInput.validation.numeric} />
						<ExpressionInput id="b2" prelabel={<M>{b}_2=</M>} size="m" settings={ExpressionInput.settings.basicTrigonometryInDegrees} validate={ExpressionInput.validation.numeric} />
					</Par>
				</InputSpace>
			</>
		},
...
function getFeedback(exerciseData) {
	...
	const numSolutionsText = [
		...
		<>Ja! Als we in de figuur de lijn van <M>{parameters.a}</M> draaien, richting de hoek van <M>{parameters.α}^\circ,</M> dan is er nog een tweede driehoek met de gegeven waarden te maken. De waarde voor <M>{parameters.b}</M> is dan kleiner.</>,
	]
