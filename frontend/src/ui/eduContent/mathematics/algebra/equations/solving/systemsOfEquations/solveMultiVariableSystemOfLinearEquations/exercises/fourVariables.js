...
const { hasVariable, hasFractionWithinFraction } = expressionChecks
...
const Problem = () => {
	...
	return <>
		<Par><Translation>Consider the system of equations <BMList><BMPart>{eq1},</BMPart><BMPart>{eq2}.</BMPart></BMList> Solve this system for <M>{variables.x}</M> and <M>{variables.y}</M>, expressing them both in <M>{variables.w}</M> and <M>{variables.z}</M>. Simplify your results as much as possible.</Translation></Par>
		<InputSpace>
			<Par>
				<ExpressionInput id="x" prelabel={<M>{variables.x}=</M>} size="s" settings={ExpressionInput.settings.withFractions} validate={ExpressionInput.validation.validWithVariables(variables)} />
				<ExpressionInput id="y" prelabel={<M>{variables.y}=</M>} size="s" settings={ExpressionInput.settings.withFractions} validate={ExpressionInput.validation.validWithVariables(variables)} />
			</Par>
		</InputSpace>
	</>
}
...
const steps = [
	...
	},
	{
		...
		Solution: ({ variables, eq2SubstitutedStep1, eq2SubstitutedStep2, eq2SubstitutedStep3, eq2SubstitutedStep4, y }) => {
			return <Par><Translation>We first get rid of the fraction: we multiply all terms by <M>{variables.a}</M>. This results in <BM>{eq2SubstitutedStep1}.</BM> Next, we expand the brackets to get <BM>{eq2SubstitutedStep2}.</BM> If we leave all terms with <M>{variables.y}</M> on the left and move all terms without <M>{variables.y}</M> to the right, then we get <BM>{eq2SubstitutedStep3}.</BM> Merging sum terms turns this into <BM>{eq2SubstitutedStep4}.</BM> The solution follows as <BM>{variables.y} = {y}.</BM></Translation></Par>
		},
	},
	{
		Problem: () => {
		...
		Solution: ({ variables, eq1, eq2, eq1Solution, x, xRaw, y }) => {
			return <Translation>
				<Par>In an earlier step we found that <BM>{variables.x} = {eq1Solution}.</BM> If we insert <M>{variables.y} = {y}</M> into this, we get <BM>{variables.x} = {xRaw}.</BM> This can be simplified to <BM>{variables.x} = {x}.</BM> The solution of the system of equations then is <M>{variables.x} = {x}</M> and <M>{variables.y} = {y}</M>.</Par>
				<Par>To check if this solution is correct, we can insert these values into the original equations. So we write
					<BMList>
					...
					If we simplify this, we see that both sides of each equation indeed come out as equal, which means that the solution is correct.</Par>
			</Translation>
		},
	},
]
...
function getFeedback(exerciseData) {
	return getFieldInputFeedback(exerciseData, {
		eq1Solution: [],
		eq2Substituted: [],
		x: [hasVariable('x'), hasVariable('y'), hasFractionWithinFraction],
		y: [hasVariable('x'), hasVariable('y'), hasFractionWithinFraction],
	})
}
