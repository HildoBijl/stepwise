...
const { originalEquation, hasSumWithinProduct, correctEquation, incorrectEquation } = equationChecks
...
const steps = [
	{
		Problem: () => {
			...
			return <>
				...
				<InputSpace>
					<Par>
						<EquationInput id="bracketsExpanded" size="l" settings={EquationInput.settings.withFractions} validate={EquationInput.validation.validWithVariables(variables)} />
					</Par>
				</InputSpace>
			</>
		},
	...
	},
	{
		...
		Solution: ({ x, termsMoved, bracketTerm, pulledOut }) => {
			return <Par><Translation>To pull <M>{x}</M> outside of brackets, we must write <M>{termsMoved.left}</M> as <M>{x}\cdot\left(\ldots\right)</M>. This tells us that there should be <M>{bracketTerm}</M> between the brackets. In this way we can rewrite the equation as <BM>{pulledOut}.</BM></Translation></Par>
		},
	},
	{
		Problem: () => {
			...
			return <>
				...
				<InputSpace>
					<Par>
						<ExpressionInput id="ans" prelabel={<M>{x}=</M>} size="l" settings={ExpressionInput.settings.withFractions} validate={ExpressionInput.validation.validWithVariables(variables)} />
					</Par>
				</InputSpace>
			</>
		},
		Solution: ({ x, bracketTerm, ans, equationWithSolution, equationWithSolutionMergedFractions, equationWithSolutionExpandedBrackets }) => {
			return <Translation>
				<Par>If we divide both sides by <M>{bracketTerm}</M>, then on the left the factor between brackets disappears. We only remain with <M>{x}</M>, meaning that we have indeed solved the equation for <M>{x}</M>! The final result is <BM>{x} = {ans}.</BM> Of course this solution can also be written in other ways, but small variations in notation are not relevant here.</Par>
