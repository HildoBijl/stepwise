...
function getFeedback(exerciseData) {
	...
	const incorrectForm = (input, correct, { equation, e, factor }, isCorrect, { translateCrossExercise }) => !isCorrect && input.findSide((side, part) => equation[part].isSum() && !(side.isProduct() && side.terms.length === (e === 1 ? 2 : 3) && (e === 1 ? side.terms.some(term => areEqualExceptOrder(term, factor)) : (side.terms.some(term => areEqualExceptOrder(factor.terms[0], term)) && side.terms.some(term => areEqualExceptOrder(factor.terms[1], term))))) && translateCrossExercise(incorrectFormMessage(part, <M>{factor} \cdot \left(\ldots\right)</M>), 'incorrectForm'))?.value
	const incorrectCopy = (input, correct, { equation }, isCorrect, { translateCrossExercise }) => !isCorrect && input.findSide((side, part) => equation[part].isSum() && !(side.isProduct() && side.terms.some(term => areEqualExceptOrder(term, equation[part]))) && translateCrossExercise(incorrectCopyMessage(part), 'incorrectCopy'))?.value
