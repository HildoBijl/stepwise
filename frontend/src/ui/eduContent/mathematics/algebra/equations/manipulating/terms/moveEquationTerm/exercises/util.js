...
export const termsLeft = (input, correct, { toLeft, ans }, isCorrect, { translateCrossExercise }) => !isCorrect && !expressionComparisons.onlyOrderChanges(input[toLeft ? 'right' : 'left'].removeUseless(), ans[toLeft ? 'right' : 'left'].removeUseless()) && translateCrossExercise(<>There are still terms left on the <Check value={toLeft}><Check.True>right</Check.True><Check.False>left</Check.False></Check> side of the equation.</>, 'termsLeft')
