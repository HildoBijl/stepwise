...
const steps = [
	...
	},
	{
		...
		Solution: ({ variables, leftExpression, rightExpression, leftAns, rightAns }) => {
			return <Par><Translation>At the first fraction we can cancel out the factor <M>{variables.y}</M>. This results in <BM>{leftExpression} = {leftAns}.</BM> For the second fraction we cancel out <M>{variables.x}</M> to find <BM>{rightExpression} = {rightAns}.</BM></Translation></Par>
		},
	},
	{
		Problem: () => {
