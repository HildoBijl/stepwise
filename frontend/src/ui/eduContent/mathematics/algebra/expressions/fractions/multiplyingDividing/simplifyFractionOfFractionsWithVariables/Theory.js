...
export function Theory() {
	return <>
		...
		<Translation entry="idea">
			<Head>The idea</Head>
			<Par>The hard part of our current situation is that we have a <Term>fraction inside of a fraction</Term>. We know how to <SkillLink skillId="multiplyDivideFractions">multiply and divide fractions with numbers</SkillLink>, and how the terms are reordered to turn any fraction of fractions into a single fraction. When using variables, it works the same way! After all, <M>\left(x+2\right)</M> is just as much a number as (for instance) the number <M>7</M>.</Par>
			<Par>Using these default rules, we may rewrite <BM>\frac(6\left(x+2\right)^2)(10\frac(\left(x+2\right)^5)(x^4)) = \frac(6\left(x+2\right)^2x^4)(10\left(x+2\right)^5).</BM> Once we have done this, we are left with a <Term>single fraction</Term>. We are already familiar with <SkillLink skillId="simplifyFractionWithVariables">simplifying a fraction</SkillLink> like this. We cross out factors, both for the number (a factor <M>2</M>) and for the variables (two factors <M>\left(x+2\right)</M>) to wind up with <BM>\frac(6\left(x+2\right)^2x^4)(10\left(x+2\right)^5) = \frac(3x^4)(5\left(x+2\right)^3).</BM> This is as simple as it can possibly be written.</Par>
		</Translation>
