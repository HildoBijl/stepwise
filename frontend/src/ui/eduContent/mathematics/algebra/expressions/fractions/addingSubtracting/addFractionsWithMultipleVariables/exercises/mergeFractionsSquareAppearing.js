...
const steps = [
	{
		...
		Solution: ({ leftExpression, rightExpression, denominator }) => {
			return <Par><Translation>The denominators <M>{leftExpression.denominator}</M> and <M>{rightExpression.denominator}</M> do not have any common factors. We can hence multiply them to get their (smallest possible) common multiple. This gives <BM>{denominator}.</BM></Translation></Par>
		},
	},
	{
		Problem: () => {
