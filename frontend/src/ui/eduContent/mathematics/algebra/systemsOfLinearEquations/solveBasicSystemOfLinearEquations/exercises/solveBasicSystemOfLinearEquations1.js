...
const Problem = () => {
	...
	return <>
		...
		<InputSpace>
			<Par>
				<ExpressionInput id="x" prelabel={<M>{variables.x}=</M>} label="Vul hier het resultaat in" size="s" settings={ExpressionInput.settings.withFractions} validate={ExpressionInput.validation.numeric} />
				<ExpressionInput id="y" prelabel={<M>{variables.y}=</M>} label="Vul hier het resultaat in" size="s" settings={ExpressionInput.settings.withFractions} validate={ExpressionInput.validation.numeric} />
			</Par>
		</InputSpace>
	</>
}
...
const steps = [
	{
		Problem: () => {
			...
			return <>
				...
				<InputSpace>
					<Par>
						<ExpressionInput id="eq1Solution" prelabel={<M>{variables.x}=</M>} label="Vul hier het resultaat in" size="s" settings={ExpressionInput.settings.withFractions} validate={ExpressionInput.validation.validWithVariables(variables)} />
					</Par>
				</InputSpace>
			</>
		},
	...
	},
	{
		...
		Solution: ({ variables, eq2SubstitutedStep1, eq2SubstitutedStep2, eq2SubstitutedStep3, eq2SubstitutedStep4, y }) => {
			return <Par>
				...
				We werken vervolgens de haakjes uit en voegen getallen samen. Zo herschrijven we de vergelijking tot
				...
				<BM>{variables.y} = {eq2SubstitutedStep4} = {y}.</BM>
			</Par>
		},
	},
	{
		...
		Solution: ({ variables, eq1, eq2, eq1Solution, x, y }) => {
			return <Par>
				...
				<BM>{variables.x} = {eq1Solution.substitute(variables.y, y)} = {x}.</BM>
				De oplossing van het stelsel van vergelijkingen is dus ook <M>{variables.x} = {x}</M> en <M>{variables.y} = {y}.</M> Als controle kunnen we deze waarden in de oorspronkelijke vergelijkingen invullen om te controleren dat de oplossing klopt. We schrijven dan
				<BMList>
				...
				En inderdaad, beide vergelijkingen kloppen.
			</Par>
		},
	},
]
...
function getFeedback(exerciseData) {
	...
	if (!progress.split) {
		...
		result.y = (input.y.value === solution.y) ? {
			type: 'success',
