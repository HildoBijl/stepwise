	...
	},
	{
		Problem: () => {
			...
			return <>
				...
				<InputSpace>
					<Par>
						<ExpressionInput id="x1" prelabel={<M>{x}_1=</M>} label="Vul hier je antwoord in" size="s" settings={ExpressionInput.settings.withRoots} validate={ExpressionInput.validation.numeric} persistent={true} />
					</Par>
				</InputSpace>
			</>
		},
		Solution: ({ x, a, b, D, x1, equationInFactors }) => {
			return <Par>
...
function getFeedback(exerciseData) {
	return {
		...
		...getFieldInputFeedback(exerciseData, ['a', 'b', 'c', 'D', 'x1']),
