		...
		Solution: ({ Tw, Tc, Qw, Qc, dSw, dSc, dS }) => {
			return <>
				<Par>Als eerste zetten we de temperaturen in Kelvin, volgens <M>T_w = {Tw}</M> en <M>T_k = {Tc}.</M> Hiermee berekenen we de entropieveranderingen van elk vat. Deze zijn
					<BMList>
						<BMPart>\Delta S_w = \frac(Q_w)(T_w) = \frac{Qw.value}{Tw.value} = {dSw},</BMPart>
						<BMPart>\Delta S_k = \frac(Q_k)(T_k) = \frac{Qc.value}{Tc.value} = {dSc}.</BMPart>
					</BMList>
					Merk op dat we een uitgaande warmtestroom negatief rekenen. De totale entropieverandering is hiermee
					<BM>\Delta S = \Delta S_w + \Delta S_k = {dSw.value} {dSc.value.texWithSign} = {dS}.</BM>
	...
	},
	{
		...
		Solution: ({ Q, Tc, Tw, dS, Wm }) => {
			...
			return <>
				...
				<Par>We hadden dit probleem ook anders op kunnen lossen. De meeste arbeid die we kunnen krijgen, in een proces met maximum temperatuur <M>T_w</M> en minimum temperatuur <M>T_k</M>, volgt vanuit een Carnot-proces. Dit proces heeft een rendement van <BM>\eta_C = 1 - \frac(T_k)(T_w) = 1 - \frac{Tc.value}{Tw.value} = {eta}.</BM> De totale geleverde warmte vanuit het warme vat is <M>Q_(toe) = {Q}.</M> Hiermee had een Carnot-proces dus een arbeid kunnen genereren van <BM>W = \eta_C Q_(toe) = {eta.value} \cdot {Q.value} = {Wm}.</BM> Dit komt op hetzelfde uit, zoals verwacht.</Par>
			</>
		},
	},
]
