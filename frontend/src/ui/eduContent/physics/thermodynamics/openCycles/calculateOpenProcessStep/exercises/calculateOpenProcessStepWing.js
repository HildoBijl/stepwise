...
const steps = [
	{
		...
		Solution: ({ rho, v1 }) => {
			return <Par>Het specifiek volume volgt vanuit de dichtheid als <BM>v_1 = \frac(1)(\rho) = \frac(1){rho.value} = {v1}.</BM></Par>
		},
	},
	{
		Problem: () => <>
		...
		Solution: ({ Rs, p1, v1, T1 }) => {
			return <Par>De gaswet voor open systemen zegt <BM>p_1v_1 = R_sT_1.</BM> De enige onbekende is <M>T_1.</M> Deze vinden we via <BM>T_1 = \frac(p_1v_1)(R_s) = \frac({p1.value} \cdot {v1.value})({Rs.value}) = {T1}.</BM> Dit is een erg koude temperatuur, maar dat is te verwachten als je op grote hoogte vliegt.</Par>
		},
	},
	{
		Problem: () => <>
		...
		Solution: ({ k, p1, p2, v1, v2, T1, T2 }) => {
			...
			if (choice === undefined || choice === 0)
				return <Par>We gaan via Poisson's wet het specifieke volume berekenen. We weten al de druk in de begin- en eindsituatie, waardoor we de wet moeten pakken met zowel <M>p</M> als <M>v.</M> Poisson's wet zegt dat <BM>p_1v_1^n = p_2v_2^n.</BM> Hierbij geldt bij een isentroop proces met lucht dat <M>n = k = {k}.</M> De oplossing voor <M>v_2</M> volgt via
					<BMList>
						...
						<BMPart>v_2 = \left(\frac(p_1)(p_2) v_1^n\right)^(\frac(1)(n)) = \left(\frac(p_1)(p_2)\right)^(\frac(1)(n)) v_1 = \left(\frac{p1.value}{p2.value}\right)^(\frac(1){k.value}) \cdot {v1.value} = {v2}.</BMPart>
					</BMList>
			...
			return <Par>We gaan via Poisson's wet de temperatuur berekenen. We weten al het volume in de begin- en eindsituatie, waardoor we de wet moeten pakken met zowel <M>p</M> als <M>T.</M> Zo vinden we dat <BM>\frac(T_1^n)(p_1^(n-1)) = \frac(T_2^n)(p_2^(n-1)).</BM> Hierbij geldt bij een isentroop proces met lucht dat <M>n = k = {k}.</M> De oplossing voor <M>T_2</M> volgt via
				<BMList>
					...
					<BMPart>T_2 = \left(T_1^n \left(\frac(p_2)(p_1)\right)^(n-1)\right)^(\frac(1)(n)) = T_1 \left(\frac(p_2)(p_1)\right)^(\frac(n-1)(n)) = {T1.value} \cdot \left(\frac{p2.value}{p1.value}\right)^(\frac({k.value}-1)({k.value})) = {T2}.</BMPart>
				</BMList>
	...
	},
	{
		...
		Solution: ({ Rs, p2, v2, T2 }) => {
			...
			if (choice === undefined || choice === 0)
				return <Par>We moeten alleen nog de temperatuur <M>T_2</M> weten. Deze vinden we via de gaswet, toegepast op punt 2. Oftewel, <BM>p_2 v_2 = R_s T_2.</BM> Dit oplossen voor <M>T_2</M> geeft <BM>T_2 = \frac(p_2v_2)(R_s) = \frac({p2.value} \cdot {v2.value})({Rs.value}) = {T2}.</BM> Dit is een stuk kouder, maar dat is logisch: bij expansie neemt de temperatuur doorgaans af.</Par>

			return <Par>We moeten alleen nog het specifieke volume <M>v_2</M> weten. Deze vinden we via de gaswet, toegepast op punt 2. Oftewel, <BM>p_2 v_2 = R_s T_2.</BM> Dit oplossen voor <M>v_2</M> geeft <BM>v_2 = \frac(R_s T_2)(p_2) = \frac({Rs.value} \cdot {T2.value})({p2.value}) = {v2}.</BM> Dit is een stuk hoger dan voorheen, maar dat is logisch: bij expansive neemt het specifieke volume toe.</Par>
		},
	},
]
