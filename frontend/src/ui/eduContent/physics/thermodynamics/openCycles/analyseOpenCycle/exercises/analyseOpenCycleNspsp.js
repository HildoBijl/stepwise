	...
	return <>
		...
		<InputSpace>
			<Par>
				...
				<QuantityInput id="Ph" prelabel={<M>P_(warm) =</M>} label="Verwarmingsvermogen" size="s" />
			</Par>
		</InputSpace>
	</>
}
...
const steps = [
		...
		Solution: ({ Rs, k, p1, v1, T1, p2, v2, T2, p3, v3, T3, p4, v4, T4 }) => {
			return <>
				<Par>In punt 1 is al bekend dat <M>p_1 = {p1}</M> en <M>T_1 = {T1}.</M> Voor punt 2 weten we <M>p_2 = {p2}.</M> We vinden <M>T_2</M> via Poisson's wet als
					...
					<BM>T_3 = T_4 \left(\frac(p_3)(p_4)\right)^(\frac(n-1)(n)) = {T4.value} \cdot \left(\frac{p3.value}{p4.value}\right)^(\frac({k}-1)({k})) = {T3}.</BM>
	...
	},
	{
		Problem: () => <>
			...
			<InputSpace>
				<Par>
					<Substep ss={1}><QuantityInput id="epsilon" prelabel={<M>\varepsilon =</M>} label="Koudefactor" size="s" validate={QuantityInput.validation.any} /></Substep>
					<Substep ss={1}><QuantityInput id="COP" prelabel={<M>\varepsilon_w =</M>} label="Warmtefactor" size="s" validate={QuantityInput.validation.any} /></Substep>
					<Substep ss={2}><QuantityInput id="Ph" prelabel={<M>P_(warm) =</M>} label="Verwarmingsvermogen" size="s" /></Substep>
				</Par>
			</InputSpace>
		</>,
		Solution: ({ mdot, wn, qin, qout, epsilon, COP, Ph }) => {
			return <>
				...
				<Par>Om het verwarmingsvermogen te vinden moeten we kijken naar hoeveel warmte wordt afgevoerd. Dit is <M>q_(af) = {qout}.</M> Vermenigvuldiging met de massastroom geeft
					<BM>P_(warm) = \dot(m) q_(af) = {mdot.value} \cdot {qout.value} = {Ph}.</BM>
