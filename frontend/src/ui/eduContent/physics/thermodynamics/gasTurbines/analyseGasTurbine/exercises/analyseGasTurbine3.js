...
const Problem = ({ p1, T1, p2, q23, etaio, mdot }) => {
	return <>
...
const steps = [
		...
		Solution: ({ k, p1, T1, p2, T2p }) => {
			return <>
				<Par>In punt 1 is al bekend dat <M>p_1 = {p1}</M> en <M>T_1 = {T1}.</M> We bereiken het fictieve punt <M>2'</M> via isentrope compressie. Met <M>p_(2') = p_2 = {p2}</M> vinden we via Poisson's wet
					<BM>T_(2') = T_1 \left(\frac(p_(2'))(p_1)\right)^(\frac(n-1)(n)) = {T1.value} \cdot \left(\frac{p2.value}{p1.value}\right)^(\frac({k}-1)({k})) = {T2p}.</BM></Par>
			</>
		},
	},
	{
		...
		Solution: ({ T1, T2, T2p, etai }) => {
			return <Par>Bij de compressor heb je vanwege frictie in werkelijkheid meer arbeid nodig dan in de perfecte (isentrope) situatie. Dit isentropisch rendement is dus
				...
				<BM>T_2 = T_1 + \frac(T_(2') - T_1)(\eta_(i_c)) = {T1.value} + \frac({T2p.value} - {T1.value})({etai.value}) = {T2}.</BM></Par>
		},
	},
	{
		Problem: () => <>
		...
		Solution: ({ cp, T2, q23, T3 }) => {
			return <Par>In de verbrandingskamer wordt de lucht isobaar verwarmd. De toegevoerde warmte is dus <BM>q_(2-3) = c_p \left(T_3 - T_2\right).</BM> Dit oplossen voor <M>T_3</M> geeft <BM>T_3 = T_2 + \frac(q_(2-3))(c_p) = {T2.value} + \frac{q23.value}{cp.value} = {T3}.</BM></Par>
		},
	},
	{
		Problem: () => <>
		...
		Solution: ({ k, p3, T3, p4, T4p }) => {
			return <Par>Omdat stap 2-3 isobaar is geldt <M>p_3 = p_2 = {p3}.</M> Net zo geldt voor stap 4-1 dat <M>p_(4') = p_4 = p_1 = {p4}.</M> We kunnen <M>T_(4')</M> nu vinden via Poisson's wet. Het resultaat is
				<BM>T_(4') = T_3 \left(\frac(p_(4'))(p_3)\right)^(\frac(n-1)(n)) = {T3.value} \cdot \left(\frac{p4.value}{p3.value}\right)^(\frac({k}-1)({k})) = {T4p}.</BM></Par>
		},
	},
	{
		Problem: () => <>
	...
	},
	{
		...
		Solution: ({ cp, T1, T2, T3, T4, wt12, wt23, wt34, wt41, wn }) => {
			return <Par>
				...
				<BM>w_(netto) = w_(t,1-2) + w_(t,2-3) + w_(t,3-4) + w_(t,4-1) = {wt12.value} {wt23.value.texWithSign} {wt34.value.texWithSign} {wt41.value.texWithSign} = {wn}.</BM>
			</Par>
		},
	},
	{
		...
		Solution: ({ P, wn, qin, eta, mdot }) => {
			return <Par>Er wordt alleen bij stap 2-3 warmte toegevoerd. De toegevoerde warmte is dus al gegeven als <M>q_(toe) = q_(2-3) = {qin}.</M> Hiermee volgt het rendement als
				...
				<BM>P = \dot(m) w_(netto) = {mdot.value} \cdot {wn.value} = {P}.</BM>
