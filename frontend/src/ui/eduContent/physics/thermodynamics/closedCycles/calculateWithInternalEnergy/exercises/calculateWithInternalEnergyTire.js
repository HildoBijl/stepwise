...
const steps = [
	{
		...
		Solution: ({ Rs, p2s, V2s, T2s, m }) => {
			return <Par>
				...
				<BM>m = \frac(p_2V_2)(R_sT_2) = \frac({p2s.value} \cdot {V2s.value})({Rs.value} \cdot {T2s.value}) = {m}.</BM>
	...
	},
	{
		...
		Solution: ({ cv, T1s, T2s, m, dU }) => {
			return <Par>
				...
				<BM>\Delta U = mc_v \left(T_2 - T_1\right) = {m.value} \cdot {cv.value} \cdot \left({T2s.value} - {T1s.value}\right) = {dU}.</BM>
