		...
		Solution: ({ m, Rs, k, p1, V1, T1, p2, V2, T2, p3, V3, T3, p4, V4, T4 }) => {
			return <>
				<Par>
					...
					<BM>m = \frac(p_1V_1)(R_sT_1) = \frac({p1.value} \cdot {p2.value})({Rs.value} \cdot {T1.value}) = {m}.</BM>
					...
					<BM>T_3 = \frac(p_3V_3)(mR_s) = \frac({p3.value} \cdot {V3.value})({m.value} \cdot {Rs.value}) = {T3}.</BM>
					...
					<BM>p_4 = p_3 \left(\frac(V_3)(V_4)\right)^n = {p3.value} \cdot \left(\frac{V3.value}{V4.value}\right)^{k} = {p4}.</BM>
		...
		Solution: ({ m, cv, T1, T2, T3, T4, Q12, W12, Q23, W23, Q34, W34, Q41, W41, Wn }) => {
			return <>
				<Par>
					...
					<BMList>
						<BMPart>Q_(2-3) = mc_v \left(T_3 - T_2\right) = {m.value} \cdot {cv.value} \cdot \left({T3.value} - {T2.value}\right) = {Q23},</BMPart>
					...
					<BMList>
						<BMPart>Q_(4-1) = mc_v \left(T_1 - T_4\right) = {m.value} \cdot {cv.value} \cdot \left({T1.value} - {T4.value}\right) = {Q41},</BMPart>
