...
const steps = [
	{
		...
		Solution: ({ m, T1, p1, Rs, p1s, V1 }) => {
			return <>
				...
				<Par>De gaswet zegt dat <BM>pV = mR_sT.</BM> Dit toepassen op punt 1 en oplossen voor <M>V_1</M> geeft <BM>V_1 = \frac(mR_sT_1)(p_1) = \frac({m.value} \cdot {Rs.value} \cdot {T1.value})({p1s.value}) = {V1}.</BM> Dit is een grote hoeveelheid, maar dat is logisch: het is het volume van <M>{m}</M> sterk verwarmde lucht.</Par>
			</>
		},
	},
	{
			...
			return <>
				<InputSpace>
					...
					{choice === 0 ? <>
						...
						<Par>
							<QuantityInput id="V2" prelabel={<M>V_2=</M>} label="Volume" size="s" />
						</Par>
					</> : null}
					{choice === 1 ? <>
	...
	},
	{
		...
		Solution: ({ Rs, m, p2s, V2, T2 }) => {
			...
			if (choice === undefined || choice === 0)
				return <Par>We moeten alleen nog de temperatuur <M>T</M> weten. Deze vinden we via de gaswet, toegepast op punt 2. Oftewel, <BM>p_2 V_2 = m R_s T_2.</BM> Dit oplossen voor <M>T_2</M> geeft <BM>T_2 = \frac(p_2V_2)(m R_s) = \frac({p2s.value} \cdot {V2.value})({m.value} \cdot {Rs.value}) = {T2}.</BM> Dit is een stuk kouder dan in de verbrandingskamer, wat logisch is: de lucht expandeert in de turbine, en bij expanderen daalt de temperatuur altijd. Hoewel een temperatuur van <M>T_2 = {T2.setUnit('dC').setDecimals(0)}</M> natuurlijk nog steeds best warm is.</Par>

			return <Par>We moeten alleen nog het volume <M>V</M> weten. Deze vinden we via de gaswet, toegepast op punt 2. Oftewel, <BM>p_2 V_2 = m R_s T_2.</BM> Dit oplossen voor <M>V_2</M> geeft <BM>V_2 = \frac(m R_s T_2)(p_2) = \frac({m.value} \cdot {Rs.value} \cdot {T2.value})({p2s.value}) = {V2}.</BM> Dit is een groter volume dan voorheen, wat logisch is: in een turbine expandeert gas waardoor de druk en de temperatuur afnemen en het volume toeneemt.</Par>
		},
	},
]
