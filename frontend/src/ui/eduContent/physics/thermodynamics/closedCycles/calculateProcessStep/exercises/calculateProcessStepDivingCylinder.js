...
import { celsiusToKelvinOffset as TConversion, barToPascalFactor as pConversion } from '@step-wise/physics-data'
...
const steps = [
	{
		...
		Solution: ({ m, p1, T1, Rs, p1s, V1, T1s }) => {
			return <>
				...
				<Par>De gaswet zegt dat <BM>pV = mR_sT.</BM> Dit toepassen op punt 1 en oplossen voor <M>V_1</M> geeft <BM>V_1 = \frac(mR_sT_1)(p_1) = \frac({m.value} \cdot {Rs.value} \cdot {T1s.value})({p1s.value}) = {V1}.</BM> Dit komt neer op <M>V_1 = {V1.setUnit('l')}</M>, wat een realistische grootte van een duikfles is. Hiermee zijn alle gaseigenschappen van de duikfles op de boot bekend.</Par>
			</>
		},
	},
	{
	...
	},
	{
		...
		Solution: ({ m, p1, T2, Rs, T2s, V2, p2 }) => {
			return <Par>We weten inmiddels dat <BM>V_2 = V_1 = {V2}.</BM> Ook is de eindtemperatuur <M>T_2</M> bekend. In standaard eenheden is deze <BM>T_2 = {T2.value} + {TConversion.value} = {T2s}.</BM> Alleen <M>p_2</M> is nog onbekend. Deze kunnen we vinden via de gaswet <BM>pV = mR_sT.</BM> Als we deze oplossen voor <M>p_2</M> vinden we <BM>p_2 = \frac(mR_sT_2)(V_2) = \frac({m.value} \cdot {Rs.value} \cdot {T2s.value})({V2.value}) = {p2}.</BM> Deze einddruk <M>p_2 = {p2.setUnit('bar').setDecimals(0)}</M> is iets minder dan de begindruk van <M>p_1 = {p1.setUnit('bar').setDecimals(0)}</M> wat logisch is: de afkoeling laat de druk iets afnemen.</Par>
		},
	},
]
