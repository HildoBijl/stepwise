...
function Solution({ type, TCold, TWarm, dTCold, dTWarm, TEvap, TCond }) {
	return <Par>We bekijken eerst de condensor. Deze zit op <M>{TCond}.</M> In de condensor gaat er warmte uit het koudemiddel (daarom condenseert het) en deze warmte wordt dus geleverd aan de {type === 'heatPump' ? 'huiskamer' : 'keuken'}. Om deze warmte te kunnen leveren moet deze ruimte dus kouder zijn dan de condensor. Zo vinden we
		<BM>{type === 'heatPump' ? `T_(binnen)` : `T_(keuken)`} = T_c - \Delta T_c = {TCond.value} - {dTWarm.value} = {TWarm}.</BM>
		...
		<BM>{type === 'heatPump' ? `T_(buiten)` : `T_(koelkast)`} = T_v + \Delta T_v = {TEvap.value} + {dTCold.value} = {TCold}.</BM>
...
function getFeedback(exerciseData) {
	...
	const { type, dTCold, dTWarm, TEvap, TCond } = parameters
	...
	if (TCold.equals(input.TWarm, compare.default) && TWarm.equals(input.TCold, compare.default))
		return {
	...
	if (TCold.equals(input.TCold, compare.default))
		feedback.TCold = true
	...
	else if (TWarm.equals(input.TCold, compare.default))
		feedback.TCold = { correct: false, text: `Dit is de temperatuur in de ${type === 'heatPump' ? 'woonkamer' : 'keuken'}.` }
	else if (wrong.TWarm.equals(input.TCold, compare.default))
	...
	if (TWarm.equals(input.TWarm, compare.default))
		feedback.TWarm = true
	else if (wrong.TWarm.equals(input.TWarm, compare.default))
	...
	else if (TCold.equals(input.TWarm, compare.default))
		feedback.TWarm = { correct: false, text: `Dit is de temperatuur in de ${type === 'heatPump' ? 'buitenlucht' : 'koelkast'}.` }
	else if (wrong.TCold.equals(input.TWarm, compare.default))
