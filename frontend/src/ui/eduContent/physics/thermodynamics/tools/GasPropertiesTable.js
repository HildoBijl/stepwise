	...
	return <>
		...
		<Table
			...
			fields={gasKeys.map(gas => [
				...
				<M>{gasProperties[gas].Rs.setUnit(gasProperties.air.Rs.unit).value}</M>,
				<M>{gasProperties[gas].k.setUnit(gasProperties.air.k.unit).value}</M>,
				<M>{gasProperties[gas].cv.setUnit(gasProperties.air.cv.unit).value}</M>,
				<M>{gasProperties[gas].cp.setUnit(gasProperties.air.cp.unit).value}</M>,
			])}
		/>
