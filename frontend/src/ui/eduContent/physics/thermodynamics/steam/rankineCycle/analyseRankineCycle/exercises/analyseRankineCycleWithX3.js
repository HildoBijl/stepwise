...
const steps = [
	...
	},
	{
		...
		Solution: ({ hx0, hx1, x3, h3 }) => {
			return <Par>Via de dampfractie <M>x_3</M> kunnen we direct de specifieke enthalpie <M>h_3</M> vinden als <BM>h_3 = h_(x=0) + x_3 \left(h_(x=1) - h_(x=0)\right) = {hx0.value} + {x3.value} \cdot \left({hx1.value} - {hx0.value}\right) = {h3}.</BM></Par>
		},
	},
	{
		Problem: ({ type }) => <>
			...
			<InputSpace>
				<Par>
					<Substep ss={1}><FloatUnitInput id="etai" prelabel={<M>\eta_i =</M>} label="Isentropisch rendement" size="s" validate={FloatUnitInput.validation.any} /></Substep>
					<Substep ss={2}><FloatUnitInput id="eta" prelabel={<M>\eta =</M>} label="Rendement" size="s" validate={FloatUnitInput.validation.any} /></Substep>
					<Substep ss={3}>{type === 1 ?
						<FloatUnitInput id="P" prelabel={<M>P =</M>} label="Geleverd vermogen" size="s" /> :
		...
		Solution: ({ type, h1, h2, h3p, h3, etai, wt, q, eta, mdot, P }) => {
			return <>
				<Par>We kunnen het isentropisch rendement van de turbine direct vinden via de definitie <BM>\eta_i = \frac(w_t)(w_t') = \frac(h_2 - h_3)(h_2 - h_(3')) = \frac({h2.value} - {h3.value})({h2.value} - {h3p.value}) = {etai}.</BM> Een isentropisch rendement van <M>{etai.setUnit('%')}</M> is redelijk realistisch voor een stoomturbine.</Par>
				<Par>Vervolgens berekenen we het (thermodynamisch) rendement van de cyclus. De technische arbeid die in de turbine geleverd wordt is <BM>w_t = h_2 - h_3 = {h2.value} - {h3.value} = {wt}.</BM> De warmte die wordt toegevoerd is <BM>q = h_2 - h_1 = {h2.value} - {h1.value} = {q}.</BM> Hiermee vinden we een rendement van <BM>\eta = \frac(\rm nuttig)(\rm invoer) = \frac(w_t)(q) = \frac{wt.value}{q.value} = {eta}.</BM> Een thermodynamisch rendement van <M>{eta.setUnit('%')}</M> is ook realistisch voor een stoomturbine.</Par>
				{type === 1 ?
