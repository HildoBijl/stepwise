...
function Solution({ a, b, c, d, e, ace, db, ans }) {
	return <Par>Om een lineaire vergelijking als deze op te lossen, willen we eerst alle termen met <M>x</M> naar links halen en alle termen zonder <M>x</M> naar rechts. Hiermee vinden we <BM>{a} x {c.texWithPM} x {e.applyMinus().texWithPM} x = {b.applyMinus()} {d.texWithPM}.</BM> Vervolgens versimpelen we deze vergelijking tot <BM>{ace} x = {db}.</BM> Als laatste stap delen we beide kanten door <M>{ace}</M> waarmee we uitkomen op <BM>x = \frac{db}{ace} = {ans}.</BM></Par>
}
