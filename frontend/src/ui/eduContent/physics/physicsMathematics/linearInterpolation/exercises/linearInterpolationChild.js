		...
		Solution: ({ type, h1, h2, W1, W2, x, h, W }) => {
			return type === 1 ? <>
				<Par>
					...
					<BM>h - h_1 = {h.value} - {h1.value} = {h.subtract(h1)}.</BM>
					De totale hoogtestijging tussen de metingen is
					<BM>h_2 - h_1 = {h2.value} - {h1.value} = {h2.subtract(h1)}.</BM>
					Het deel van de eerste, ten opzichte van de tweede, is
					<BM>x = \frac(h - h_1)(h_2 - h_1) = \frac({h.value} - {h1.value})({h2.value} - {h1.value}) = \frac{h.subtract(h1).value}{h2.subtract(h1).value} = {x}.</BM>
					Dit is dus het deel van de hoogtestijging.
				</Par>
			</> : <>
				<Par>De gewichtstoename sinds de eerste meting is
					<BM>W - W_1 = {W.value} - {W1.value} = {W.subtract(W1)}.</BM>
					De totale gewichtstoename tussen de metingen is
					<BM>W_2 - W_1 = {W2.value} - {W1.value} = {W2.subtract(W1)}.</BM>
					Het deel van de eerste, ten opzichte van de tweede, is
					<BM>x = \frac(W - W_1)(W_2 - W_1) = \frac({W.value} - {W1.value})({W2.value} - {W1.value}) = \frac{W.subtract(W1).value}{W2.subtract(W1).value} = {x}.</BM>
		...
		Solution: ({ type, h1, h2, W1, W2, x, h, W }) => {
			return type === 1 ?
				<Par>
					...
					<BM>x \left(W_2 - W_1\right) = {x} \cdot \left({W2.value} - {W1.value}\right) = {W2.subtract(W1).multiply(x)}.</BM>
					Dit is de toename sinds de eerste meting. Het gewicht zelf is dus
					<BM>W = W_1 + x \left(W_2 - W_1\right) = {W1.value} + {x} \cdot \left({W2.value} - {W1.value}\right) = {W}.</BM>
				...
				<Par>
					...
					<BM>x \left(h_2 - h_1\right) = {x} \cdot \left({h2.value} - {h1.value}\right) = {h2.subtract(h1).multiply(x)}.</BM>
					Dit is de stijging sinds de eerste meting. De hoogte zelf is dus
					<BM>h = h_1 + x \left(h_2 - h_1\right) = {h1.value} + {x} \cdot \left({h2.value} - {h1.value}\right) = {h}.</BM>
