...
const Problem = ({ angle, horizontal, FD }) => {
	return <>
...
const steps = [
	...
	},
	{
		...
		Solution: ({ intersection, A, D, up, right, horizontal, isFAInPositiveDirection, FDx, FDy, rDx, rDy, rA, FA }) => {
			...
			return <Par>
				...
				<BM>F_A = \frac({FAPositive === FDxPositive ? '-' : ''} r_(Dx) F_(Dx) {FAPositive === FDyPositive ? '-' : '+'} r_(Dy) F_(Dy))(r_A) = \frac({FAPositive === FDxPositive ? '-' : ''} {rDx.value} \cdot {FDx.value} {FAPositive === FDyPositive ? '-' : '+'} {rDy.value} \cdot {FDy.value})({rA.value}) = {FA}.</BM>
...
function getFeedback(exerciseData) {
	...
	return {
		...
		...getFieldInputFeedback(exerciseData, ['FDx', 'FDy', 'FA']),
	}
}
