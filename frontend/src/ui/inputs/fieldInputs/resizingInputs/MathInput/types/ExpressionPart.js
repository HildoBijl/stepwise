...
export function keyPressToFI(keyInfo, FI, settings, charElements, topParentFI, contentsElement, cursorElement) {
	...
	if (key === '(') {
		...
		const netBracketsBefore = ExpressionFunctions.countNetBrackets(parentExpressionFI, -1)
		const netBracketsAfter = ExpressionFunctions.countNetBrackets(parentExpressionFI, 1)
