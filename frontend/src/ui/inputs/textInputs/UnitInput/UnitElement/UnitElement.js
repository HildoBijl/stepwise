	...
	return (
		<span className={clsx('unitElement', { valid, invalid: !valid })}>
			<span className="prefix">
				<CharString str={value.prefix} cursor={cursor?.part === 'text' && cursor.cursor <= value.prefix.length && cursor.cursor} />
			</span>
			<span className="baseUnit">
				{useFiller ?
					<span className={clsx('char', 'filler', 'filler-qm')} key='filler'>?</span> :
					<CharString str={value.unit} cursor={cursor?.part === 'text' && cursor.cursor > value.prefix.length && cursor.cursor <= value.prefix.length + value.unit.length && cursor.cursor - value.prefix.length} />
				}
			</span>
			<span className="power">
				<CharString str={value.power} cursor={cursor?.part === 'power' && cursor.cursor} />
			</span>
		</span>
	)
}
