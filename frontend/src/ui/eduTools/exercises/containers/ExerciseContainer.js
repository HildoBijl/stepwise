...
export function ExerciseContainer({ exercise, groupExercise, submitting, submitAction, cancelAction, resolveEvent, startNewExercise, example }) {
	...
	const reload = () => {
		...
		setLoading(true)
		Promise.all([
			import(/* webpackChunkName: "front-end-exercises-56" */ `ui/eduContent/${exercises[exerciseId].path.join('/')}/exercises/${exerciseName}`),
			import(/* webpackChunkName: "shared-exercises-56" */ `step-wise/eduContent/${exercises[exerciseId].path.join('/')}/${exerciseName}`),
