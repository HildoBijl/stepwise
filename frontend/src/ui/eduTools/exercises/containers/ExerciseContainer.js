...
export function ExerciseContainer({ exercise, groupExercise, submitting, submitAction, cancelAction, resolveEvent, startNewExercise, example }) {
	...
	const reload = () => {
		setLoading(true)
		Promise.all([
			import(/* webpackChunkName: "front-end-exercises-21" */ `ui/eduContent/${exercises[exerciseId].path.join('/')}/exercises/${exerciseId}`),
			import(/* webpackChunkName: "shared-exercises-21" */ `step-wise/eduContent/${exercises[exerciseId].path.join('/')}/${exerciseId}`),
