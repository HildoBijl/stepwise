...
const event = (eventIndex: number): GroupExerciseEventRecord => ({ id: `event-${eventIndex}`, eventIndex, state: null, report: null, performedAt: '', actions: [] })
...
describe('mergeGroupEventResolution', () => {
	it('resolves the current event and appends the next event', () => {
		...
		const resolution: GroupEventResolutionRecord = { exerciseId: 'exercise', eventIndex: 0, state: {}, report: { user: { correct: true } }, active: true, nextEvent }
		const result = mergeGroupEventResolution(exercise, resolution, vi.fn())
		expect(result).toMatchObject({ eventIndex: 1, active: true, history: [{ eventIndex: 0, state: {}, report: { user: { correct: true } } }, { ...nextEvent, actions: [] }] })
	})

	it('refetches when the next event is not sequential', () => {
		const refetch = vi.fn(async () => undefined)
		const { actions: _actions, ...nextEvent } = event(2)
		const resolution: GroupEventResolutionRecord = { exerciseId: 'exercise', eventIndex: 0, state: {}, report: null, active: true, nextEvent }
